...
PermissionList=Ext.extend(BaseGridList,{
	...
	initComponent : function(){
		...
		{header: "权限名", sortable:true,width: 100, dataIndex:"name"},
		{header: "简介", id:"description",sortable:true,width: 165, dataIndex:"description"}
        ])
...
if (typeof RoleList === "undefined") {
	...
	initComponent : function(){
		this.cm=new Ext.grid.ColumnModel([
		{header: "编码", sortable:true,width: 80, dataIndex:"name"},
		{header: "名称", sortable:true,width: 165, dataIndex:"title"}
        ])
...
// 系统限资源管理
SystemResourceListPanel = Ext.extend(Disco.Ext.CrudPanel, {
			id : "systemResourceListPanel",
			
			//title : "系统权限资源管理",
			baseUrl : "resource.java",
			batchRemoveMode:true,			
			pageSize:25,
			viewWin :{title:"资源详情查看",width:600,height:480},
			...
			createViewPanel : function() {
				if (!this.viewPermissionGrid) {
					this.viewPermissionGrid = new PermissionList({
						...
						gridForceFit:false,
						pagingToolbar:false,
						url : "resource.java?cmd=loadPermission"
					});
				}
				if(!this.viewRoleGrid){
					this.viewRoleGrid=new RoleList({
						...
						height:240,
						pagingToolbar:false,
						url : "resource.java?cmd=loadRole"
					});
				}
					...
					items : [
					{
						...
						items : [
							...
							}]},
							{
								...
								layout : "fit",						
								items:{
									layout:"column",items:[this.viewPermissionGrid,{width:5},this.viewRoleGrid]	
								}
							}
							]
				});
			...
			view : function() {
				...
				if (ret) {
					...
					this.viewPermissionGrid.store.load({params : "id="
			...
			importModuleResource:function(){
				Ext.Ajax.request({
					...
					params:{pack:this.pack,action:this.action},
					success:function(){
						this.refresh();
					},
					scope:this
...
SystemResourcePanel = function() {
	...
	this.tree.on("click", function(node, eventObject) {
		...
		Ext.apply(this.list.store.baseParams,{action:this.list.action,pack:this.list.pack});
...
Ext.extend(SystemResourcePanel, Ext.Panel,{
	...
	createBatchPermissionPanel : function(){
		...
		return new Ext.grid.GridPanel({
			...
			store : new Ext.data.JsonStore({
				data : [],
				fields : ['id','type','desciption','resStr']
			})
		});
	},
	beforeDestroy:function(){
	...
	batchCreatePermission : function(){
		if(this.currentNode){
			this.batchPermissionPanel =  this.batchPermissionPanel || this.createBatchPermissionPanel();
