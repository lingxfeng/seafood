...
if (!Global.systemRoleCheckLoader) {
	Global.systemRoleCheckLoader =new Disco.Ext.MemoryTreeLoader({
		iconCls : 'disco-tree-node-icon',
		varName:"Global.SYSTEM_ROLE_CHECK_NODES",
		url : "role.java?cmd=getTree&pageSize=-1&treeData=true&all=true&checked=false",
		listeners : {
			'beforeload' : function(treeLoader, node) {
				treeLoader.baseParams.id = (node.id.indexOf('root')<0 ? node.id : "");
				if(typeof node.attributes.checked!=="undefined"){
...
//系统菜单管理
SystemMenuListPanel = Ext.extend(Disco.Ext.CrudPanel, {
	//title:"菜单详情",
	...
	createForm : function() {
		var formPanel = new Ext.form.FormPanel({
			...
			items : [{
				...
				border:false,
				activeTab:0,
				items:[{title:'系统菜单',border:false,layout:'form',items:[{
						xtype : "hidden",
						name : "id"
					},
					{
					...
					{xtype:"numberfield",fieldLabel : "费用",name : "fee"},
					Disco.Ext.Util.buildCombox("status","状态",this.theStatus,0),
					{xtype:"numberfield",fieldLabel : "显示顺序",name : "sequence"}
					),
					{
					anchor : '98%',
					...
					{xtype:"textfield",anchor : '-20',fieldLabel : "脚本地址",name : "url"},
					{xtype:"textfield",anchor : '-20',fieldLabel : "依赖脚本地址",name : "otherScripts"},
					{xtype:"textfield",anchor : '-20',fieldLabel : "图标",name : "icon"},
					{xtype:"textarea",anchor : '-20',fieldLabel : "参数信息",name : "params",height:40}
					]
					}]},{
						title:'功能列表',
						cmd:'permission',
						layout:'fit',
						hideBorders:true,
						bbar:[{text:'添加权限',handler:this.addPermission,scope:this},{text:'删除权限',handler:this.delPermission,scope:this}],
	...
	createGridPanel:function(){
		...
		this.permissionGrid =new Ext.grid.GridPanel({
			cm:new Ext.grid.ColumnModel([sm,{header:'权限名称',dataIndex:'id',hidden:true},{header:'权限名称',dataIndex:'name'},{header:'简介',dataIndex:'description'}]),
			sm:sm,
	...
	addPermission:function(){
		if(!this.selectWin){
			Disco.Ext.Util.listObject('PermissionPanelManage',function(o){
				this.selectWin= new Ext.Window({
					...
					buttons:[{text:'保存',handler:function(){
						...
						if(records.length>0){
							Ext.each(records,function(r){
								if(this.permissionGrid.getStore().indexOfId(r.get('id'))<0)
									this.permissionGrid.getStore().add(r);	
							},this)
						}else{
							Ext.Msg.show({
								width:200,
						...
						this.selectWin.hide();
					},scope:this},{text:'取消',handler:function(){this.selectWin.hide()},scope:this}]
				});
				this.selectWin.show();
	...
	reCalMenuAndRoles:function(){
		...
		if(!record){
			...
			Ext.Ajax.request({url:"securityHelper.java",params:{cmd:"reCalSystemMenu",id:record.get("id")},scope:this,success:function(req){
    ...
    reloadTree:function(){
        ...
        if(this.fp){
            ...
            if(parentNode&&parentNode.tree.rendered)parentNode.tree.root.reload();
        }
    },
	save : function(callback,autoClose) {
		...
		var permissions=Disco.Ext.Util.getGridDataAsString(this.permissionGrid);
		Ext.apply(this.fp.form.baseParams,{permissions:permissions});
	...
	storeMapping : ["id","sn", "title","url","vrtype","roles","permissions","types","appClass","sequence","status","parent",{name:"parentId",mapping:"parent"},"otherScripts","icon","pack","theRoles","fee","system","params"],
