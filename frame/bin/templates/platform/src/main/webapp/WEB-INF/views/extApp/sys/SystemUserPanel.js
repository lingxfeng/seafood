if (typeof Global === "undefined") {
	Global = {
		departmentLoader : new Ext.tree.TreeLoader({
					url : "department.java?cmd=getDepartment&pageSize=-1&treeData=true",
					listeners : {
 ...
 */
SystemUserPanel = Ext.extend(Disco.Ext.CrudPanel, {
	pageSize : 17,
	baseUrl : "systemUser.java",
	createForm : function() {
		if (!this.resourceGrid) {
			this.resourceGrid = new PermissionList({
						...
						height : 150,
						border : false,
						url : "permission.java?cmd=list&pageSize=-1"
					});
		}
		if (!this.selectGrid) {
			this.selectGrid = new PermissionList({
						...
						height : 150,
						border : false,
						url : "employee.java?cmd=loadPermission"
					});
		...
		if (!this.resourceRoleGrid) {
			this.resourceRoleGrid = new RoleList({
						...
						height : 180,
						border : false,
						url : "role.java?cmd=list&pageSize=-1"
					});
		}
		if (!this.selectRoleGrid) {
			this.selectRoleGrid = new RoleList({
						...
						height : 180,
						border : false,
						url : "employee.java?cmd=loadRole"
					});
	...
	storeMapping : ["id", "name", "trueName", "tel","email","lastLoginTime","lastLoginIP","loginTimes","remark", "password", "tel", "registerTime", "remark","status"],
	initComponent : function() {
		this.cm = new Ext.grid.ColumnModel([{
					header : "用户名",
					sortable : true,
