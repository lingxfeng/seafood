...
if (!Global.permissionGroupLoader) {
    Global.permissionGroupLoader = new Ext.tree.TreeLoader({
        ...
        listeners: {
            'beforeload': function(treeLoader, node) {
                treeLoader.baseParams.permissionId = (node.id.indexOf('root') < 0 ? node.id : "");
            }
        }
    })
}
...
ResourceList = Ext.extend(BaseGridList, {
    ...
    initComponent: function() {
        ...
        this.typesItem = new Ext.form.ComboBox(Ext.apply({}, {
            ...
            emptyText: '请选择...',
            editable: false
        }, Disco.Ext.Util.buildCombox("types", "types", this.types)));
    ...
    createForm: function() {
        ...
        this.editGrid = new Ext.grid.GridPanel({
            ...
            bbar: [{
                ...
                handler: this.choiceSelectGridData("editGrid", "selectWin", "ResourceList", "选择资源"),
                scope: this
            }, {
                text: "删除资源",
                cls: "x-btn-text-icon",
                ...
                handler: function() {
                    Disco.Ext.Util.removeGridRows(this.editGrid);
                },
                scope: this
            }]
        });
    ...
    view: function() {
        ...
        if (ret) {
            ...
            this.viewSelectGrid.store.load({
                params: "id=" + this.grid.getSelectionModel().getSelected().get("id")
            });
...
PermissionManagePanel = Ext.extend(Ext.Panel, {
    layout: 'border',
    border: false,
                    ...
                    Ext.Msg.prompt("新增权限", "请输入新增权限名称", function(btn, text) {
                        if (btn == 'ok') {
                            ...
                            Ext.Ajax.request({
                                ...
                                params: {
                                    name: text,
                                    parent: node.attributes.id == "root" ? "" : node.attributes.id
                                },
