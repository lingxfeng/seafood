...
/**
 * 待审核中的文章
 * 分行管理员待审核的文档（其实操作功能有：已阅读、发布、采纳）
 * @class BranchAfterAuditingDocManagePanel
 * @extends Disco.Ext.CrudPanel
 */
BranchAfterAuditingDocManagePanel = Ext.extend(Disco.Ext.CrudPanel, {
    gridSelModel: 'checkbox',
    id: "branchAfterAuditingDocManagePanel",
     ...
     * 选择一个栏目并且发布文档
     */
    selectDirAndOpenDoc: function() {
        var record = this.currentRecord;
        if (!this.selectDirAndOpenDocWin) {
            this.selectDirAndOpenDocWin = this.createSelectDirAndOpenDocWin();
        }
        this.selectDirAndOpenDocWin.show();
        var form = this.selectDirAndOpenDocWin.getComponent(0).form;
        form.clearData();
        form.reset();
        form.loadRecord(record);
    },
    createSelectDirAndOpenDocWin: function() {
            ...
            buttons: [{
                ...
                handler: function() {
                    ...
                    me.executeUrl('newsDoc.java', {
                        ...
                        cmd: 'branchOpenDoc',
                        branchDirId: me.getViewFormObj().branchDirId
                    }, function() {
                        me.grid.store.reload()
                        me.viewWinPanel.hide();
                        me.selectDirAndOpenDocWin.hide();
                    })();
                },
                iconCls: 'save',
                scope: this
    ...
    createViewWinPanel: function() {
        this.currentRecord = this.grid.getSelectionModel().getSelected();
        var win = new Ext.Window({
            width: 900,
            height: 620,
            closeAction: 'hide',
            ...
            buttons: [{
                text: "发布(<u>K</u>)",
                handler: this.selectDirAndOpenDoc,
                iconCls: 'save',
                scope: this
            }, {
                text: "采纳(<u>X</u>)",
    ...
    initComponent: function() {
        ...
        BranchAfterAuditingDocManagePanel.superclass.initComponent.call(this);
    }
});