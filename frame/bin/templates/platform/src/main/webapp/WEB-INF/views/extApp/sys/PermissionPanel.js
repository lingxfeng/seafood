...
if (!Global.permissionGroupLoader) {
	Global.permissionGroupLoader = new Ext.tree.TreeLoader({
		iconCls : 'disco-tree-node-icon',
		url : "permissionGroup.java?cmd=getTree",
		listeners : {
			'beforeload' : function(treeLoader, node) {
...
if (typeof RoleList === "undefined") {
	...
	initComponent : function(){
		...
		chkM,
		{header: "编码", sortable:true,width: 60, dataIndex:"name"},
		{header: "名称", sortable:true,width: 120, dataIndex:"title"},
		{header: "简介", sortable:true,width: 100, dataIndex:"description"}
        ])
...
ResourceList=Ext.extend(BaseGridList,{
	types : [["模块限制", "MODULE"],["URL限制", "URL"], ["方法限制", "METHOD"], ["ACL限制", "ACL"]],
   	storeMapping:["id","resStr","type","desciption"],
   	loadData:false,
   	border : false ,
   	url:"resource.java?cmd=list",
   	reset:function(){
	...
	resourceSearch:function(){
		...
		this.grid.store.params={start:0,action:this.action,type:this.typesItem.getValue(),searchKey:this.searchKey.getValue()};
		this.grid.store.reload();
	},
	initComponent : function(){
		var chkM=new Ext.grid.CheckboxSelectionModel();
		...
		this.cm=new Ext.grid.ColumnModel([	
		chkM,
		{header: "资源类型", sortable:true,width: 70, dataIndex:"type"},
		{header: "资源描述", sortable:true,width: 300, dataIndex:"resStr"},
		{header: "简介", sortable:true,width: 80, dataIndex:"desciption"}
        ]);
        this.moduleItem=new Disco.Ext.TreeComboField({
				displayField:"title",
				width:220,
				listWidth:250,
				 ...
				 listeners:{
					select:function(c,node){
						if(!node.isLeaf()&&node.id!="root"){
							...
							this.pack="";
							this.action=node.id=="root"?"":node.id;
						}
						this.resourceSearch();
					 },
					scope:this	
			...
			this.typesItem=new Ext.form.ComboBox(Ext.apply({},{width:80,emptyText : '请选择...',editable : false},Disco.Ext.Util.buildCombox("types","types",this.types)));
			this.searchKey=new Ext.form.TextField({
				name:"searchKey",
				width:80
			});
		this.tbar=["模块：",this.moduleItem,"资源类型:",this.typesItem,"关键字:",this.searchKey,"->",{
								text : "查询",
...
// 系统权限
PermissionPanel = Ext.extend(Disco.Ext.CrudPanel, {
	id : "permissionPanel",
	pageSize:20,
	importData:true,
	exportData:true,
	...
	viewWin :{title:"权限详情查看",width:600,height:480},
	createSelPermissionGroup:function(){
		...
		if(!this['selPermissionGroup']){
			this['selPermissionGroup']= new Ext.tree.TreePanel({
					...
					autoScroll:true,
					width : 250,
					bbar:['->',{text:"刷新",handler:function(btn){
						this['selPermissionGroup'].getRootNode().reload();
					},scope:this}],
					root:new Ext.tree.AsyncTreeNode({
						id:'root',
						text:'权限组',
						...
						loader:new Ext.tree.TreeLoader({
							...
							listeners:{
								beforeload:function(treeLoader,node){
									treeLoader.baseParams.permissionId=(node.id!='root'?node.attributes.id:'');
								},
								scope:this
							}
						})
			...
			handler:function(btn,win,tree){
				...
				if(btn=='yes'){
					if(node){
						...
						var parentId = node.attributes.id=='root'?'':node.attributes.id;
						this.moveToPermissionGroup({parentId:parentId,id:ids});
					}
				}
			},
			scope:this
	...
	moveToPermissionGroup:function(params){
		Ext.Ajax.request({
			url:'permission.java?cmd=moveToPermissionGroup',
			params:params,
			success:function(){
	...
	choiceSelectGridData:function(grid,winName,gridList,winTitle){
		return function(){
			...
			if(!this[winName]){
				...
				this[winName]=new Disco.Ext.GridSelectWin({border:false,title:winTitle,width:650,grid:glist});
				this[winName].on("select", function(datas) {
					var ds=[];
					for(var i=0;i<datas.length;i++){//过滤掉重复的内容
						if(theGrid.store.find("id",datas[i].id)<0)ds[ds.length]=datas[i];
					}
	...
	createViewPanel:function(){
		if (!this.viewSelectGrid) {
			this.viewSelectGrid = new ResourceList({
				...
				gridForceFit:false,
				pagingToolbar:false,
				url : "permission.java?cmd=loadResource"
			});
		}
		if(!this.viewRoleGrid){
			this.viewRoleGrid=new RoleList({
				...
				height:250,
				pagingToolbar:false,
				url : "permission.java?cmd=loadRole"
			});
		}
		var formPanel = new Ext.form.FormPanel({
			...
			items : [{
						...
						layout : "fit",						
						items:{
							layout:"column",items:[this.viewSelectGrid,{width:5},this.viewRoleGrid]	
						}
					}]
		});
		return formPanel;
		...
		this.editGrid = new Ext.grid.GridPanel({
			...
			cm:new Ext.grid.ColumnModel([
					 chkM,
			 		 ...
			 		 {header: "资源类型", sortable:true,width: 70, dataIndex:"type"},					
					 {header: "资源描述", sortable:true,width: 300, dataIndex:"resStr"},
					  {header: "简介", sortable:true,width: 80, dataIndex:"desciption",hidden:true}
			]),
			store:new  Ext.data.JsonStore({
				fields:["id","type","description","resStr"]
			}),
			bbar:[{text:"添加资源",cls : "x-btn-text-icon",
				icon : "images/icons/application_form_add.png",handler:this.choiceSelectGridData("editGrid","selectWin","ResourceList","选择资源"),scope:this},{text:"删除资源",cls : "x-btn-text-icon",
				icon : "images/icons/application_form_delete.png",handler:function(){Disco.Ext.Util.removeGridRows(this.editGrid);},scope:this}]
		});	
		...
		this.roleGrid = new Ext.grid.GridPanel({
			...
			cm:new Ext.grid.ColumnModel([
					 chkM,
			 		 new Ext.grid.RowNumberer({header:"序号",width:40}),	 		 
			 		 {header:"ID",dataIndex:"id",hideable:true,hidden:true},
			 		 {header: "角色编码", sortable:true,width: 70, dataIndex:"name"},					
			...
			bbar:[{text:"添加角色",cls : "x-btn-text-icon",
				icon : "images/icons/application_form_add.png",handler:this.choiceSelectGridData("roleGrid","selectRoleWin","RoleList","选择角色"),scope:this},{text:"删除角色",cls : "x-btn-text-icon",
				icon : "images/icons/application_form_delete.png",handler:function(){Disco.Ext.Util.removeGridRows(this.roleGrid);},scope:this}]
		});
		var formPanel = new Ext.form.FormPanel({
			frame : true,
			labelWidth : 60,
			...
			items : [{
					...
					tree : new Ext.tree.TreePanel({
						...
						height : 300,
						layout : "fit",
						items : {xtype:"tabpanel",activeTab:0,items:[this.editGrid,this.roleGrid]}
					}]
		});
		return formPanel;
	},	
	...
	statusRender:function(v){
		if(v==-1)return "<font color=red>停用</a>";
		else return "启用";
	},
	onCreate : function() {
	...
	view : function() {
		...
		if (ret) {
			...
			this.viewSelectGrid.store.load({params : "id="
						+ this.grid.getSelectionModel().getSelected().get("id")});
			this.viewRoleGrid.store.removeAll();
			this.viewRoleGrid.store.load({params : "id="
						+ this.grid.getSelectionModel().getSelected().get("id")});			
		}
	},
	onEdit : function(ret) {
		if (ret) {
			this.editGrid.store.removeAll();
			var items=this.grid.getSelectionModel().getSelected().get("resources")
			this.editGrid.store.removeAll();
	...
	save : function() {
		...
		this.fp.form.baseParams={resources:rs,roles:roles};
	...
	storeMapping : ["id", "name", "operation", "resources", "description","status","roles","parent"],
	initComponent : function() {
		...
		this.cm = new Ext.grid.ColumnModel([sm,{
					...
					sortable : true,
					width : 80,
					dataIndex : "status",renderer:this.statusRender
				}]);
		this.gridConfig = {enableDragDrop:true,sm:sm,ddGroup:"gridDD"};
		this.gridButtons=[{text:'移动到权限组',iconCls:'move',handler:this.createSelPermissionGroup,scope:this}],
...
PermissionPanelManage = Ext.extend(Ext.Panel,{
	...
	onClickNode:function(node,e){
		this.grid.store.baseParams={permissionId:node.id!='root'?node.attributes.id:'',searchKey:""};
				...
				handler:function(item){
					...
					Ext.Msg.prompt("新增权限","请输入新增权限名称",function(btn,text){
						if(btn=='ok'){
							...
							Ext.Ajax.request({
								scope:this,
								url:'permissionGroup.java?cmd=save',
								params:{name:text,parent:node.attributes.id=="root"?"":node.attributes.id},
				...
				handler:function(item){
					...
					Ext.Msg.prompt("修改权限","请输入修改权限名称",function(btn,text){
						if(btn=='ok'){
							...
							Ext.Ajax.request({
								scope:this,
								url:'permissionGroup.java?cmd=update',
								params:{name:text,id:node.attributes.id},
								success:function(response){
									node.setText(text);
								}
							});
				...
				handler:function(item){
					...
					Ext.Msg.confirm("操作提示","你確定要刪除該权限吗!",function(btn){
						if(btn=='yes'){
							Ext.Ajax.request({
								scope:this,
								url:'permissionGroup.java?cmd=remove',
	...
	dragdrop:function(tree, node, oldParent, newParent) {
		var o = {
			...
			newParentId:newParent.attributes.id=='root'?"":newParent.attributes.id
		};
		Ext.Ajax.request({
			url:'permissionGroup.java?cmd=move',
			params:o,
			...
			listeners:{
				...
				beforenodedrop:function(e){
					...
					if(e.data.grid){
						...
						var parentId = node.attributes.id=='root'?'':node.attributes.id;
						this.permission.moveToPermissionGroup({parentId:parentId,id:ids});
					}
				},
				scope:this
			}
	...
	initComponent:function(){
		...
		this.permission =new PermissionPanel({region:'center',border:false});
