...
TreeListDD=Ext.extend(Ext.Panel,{
	layout:"border",
	frame:false,
	listeners:{
		scope:this,
	...
	initComponent:function(){
		TreeListDD.superclass.initComponent.call(this);		
		this.requirementTree = new Ext.ux.tree.TreeGrid({
			region : "north",
			width : 500,
			height : 200,
			frame:false,
			...
			listeners:{
				scope:this,
				click:function(n,e){
					this.stepWorkGroupList.updateContent.call(this.stepWorkGroupList,n.id);
				},
				beforenodedrop:function(de){
					var node=de.dropNode.attributes;
					var target=de.target.attributes;
					Disco.Ext.Util.ajaxRequest("treeListDD.ejf",{cmd:"swapRequirement",nodeId:node.id,targetId:target.id},this);
				}
			},
			columns : [
				{
		...
		this.stepWorkGroupList=new Ext.grid.GridPanel({
			region:"center",
			frame:false,
		    width: 400,
		    height: 200,
		    ...
		    listeners:{
		    	scope:this,
		    	render:function(gv){
		    		var owner=this.stepWorkGroupList;
					gv.dropTarget = new Ext.dd.DropTarget(gv.getView().scroller.dom,{
						...
						notifyDrop : function(ddSource, e, data){
							...
							if(!owner.reqId)
								return false;
							owner.dropRelatedObjectHanlder.call(owner,params);
		                    return true;
		                }
					});
				}
		    ...
		    columns: [
		        ...
		        {header: "加入时间", width: 80, sortable: true, dataIndex: 'addDate',renderer:Disco.Ext.Util.dateRender("Y-m-d G:i")},
		...
		this.stepWorkList=new Ext.grid.GridPanel({
			region:"east",
			width:100,
			height:200,
			border:1,
			...
			listeners:{
		    	...
		    	render:function(gv){
					...
					gv.dropTarget = new Ext.dd.DropTarget(gv.getView().scroller.dom,{
						...
						notifyDrop : function(ddSource, e, data){
							data=data.selections[0].data;
							owner.stepWorkList.dropRelatedObjectHanlder.call(owner,data);
		                    return true;
		                }
					});
				}
		    ...
		    dropRelatedObjectHanlder:function(params){
		    	Disco.Ext.Util.ajaxRequest(
		    		...
		    		function(resp){
		    			console.debug(this);
		    			var ret=Ext.decode(resp.responseText);
		    			this.stepWorkGroupList.store.removeAll();
		    			this.stepWorkGroupList.store.loadData(ret);
		    		}
		    	);
		    },
		...
		this.add(this.requirementTree,this.stepWorkGroupList,this.stepWorkList);
	}
});
