...
if(!Global.requirementLoader){
	Global.requirementLoader =new Ext.tree.TreeLoader({
		...
		url : "/treeListDD.java?cmd=getRequirementTree&pageSize=-1&treeData=true",
		listeners : {
...
Ext.onReady(function() {
	var requirementTree = new Ext.ux.tree.TreeGrid({
		...
		listeners:{
			...
			beforenodedrop:function(de){
				...
				Disco.Ext.Util.ajaxRequest("/treeListDD.java",{cmd:"swapRequirement",nodeId:node.id,targetId:target.id},this);
			}
		},
		columns : [
			{
	...
	var stepWorkList=new Ext.grid.GridPanel({
		...
		store : new Ext.data.JsonStore({
			url : "/systemDictionary.java?cmd=getDictionaryBySn&sn=StepWork",
			root : "result",
			totalProperty : "rowCount",
			autoLoad:true,
			remoteSort : true,
