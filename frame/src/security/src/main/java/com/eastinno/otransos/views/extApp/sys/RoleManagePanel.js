...
if (!Global.systemRoleLoader) {
    Global.systemRoleLoader =new Ext.tree.TreeLoader({
       // varName:"Global.SYSTEM_ROLE_NODES",
        url : "role.java?cmd=getTree&pageSize=-1&treeData=true&all=true",
...
if (typeof PermissionList === "undefined") {
    PermissionList=Ext.extend(BaseGridList,{
        ...
        quickSearch:function(){
            ...
            this.grid.store.reload({params:{searchKey:this.btn_search.getValue(),start:0}});
        },
        initComponent : function(){
            ...
            chkM,
            {header: "权限名", sortable:true,width: 60, dataIndex:"name"},
            {header: "简介", sortable:true,width: 100, dataIndex:"description"}
            ]);      
           this.btn_search=new Ext.form.TextField({width:100});
           this.tbar=["关键字:",this.btn_search,{text:"查询",handler:this.quickSearch,scope:this,cls:"x-btn-text-icon",icon:"images/icon-png/search.png"}];
...
// 角色管理
RolePanel = Ext.extend(Disco.Ext.CrudPanel, {
    id : "rolePanel",
    //title : "系统角色管理",
    baseUrl : "role.java",
    theStatus : [["启用", 0],["停用", -1]],
    ...
    choicePermission:function(){
        if(!this.selectWin){
            this.selectWin=new Disco.Ext.GridSelectWin({title:"选择权限",grid:new PermissionList()});
            this.selectWin.on("select", function(datas) {
                var ds=[];
                for(var i=0;i<datas.length;i++){//过滤掉重复的内容
                    if(this.editGrid.store.find("id",datas[i].id)<0)ds[ds.length]=datas[i];
                }
    ...
    createForm : function() {
            ...
            renderer : (function(v, p, record) {
                ...
                if(r){
                    ...
                    if(Ext.isArray(allPermissions) && Ext.isArray(permissions)){
                        ...
                        var length = Math.min(allPermissions.length,permissions.length);
                        if(length==0){
                            length = Math.max(allPermissions.length,permissions.length);
                        }
        ...
        cm:new Ext.grid.ColumnModel([
                 ...
                 {header:"权限名称",dataIndex:"name",width:100,
                 renderer : (function(v, p, record) {
                    var r = this.grid.getSelectionModel().getSelected();
                    if(r){
                        var allPermissions = r.get('allPermissions');
                        ...
                        if(Ext.isArray(allPermissions) && Ext.isArray(permissions)){
                            ...
                            for (var i = 0,allLen = allPermissions.length,Len = permissions.length; i < length; i++) {
                                if(i<Len&&permissions[i].id==record.get('id')){
                                    isExist=true;
                                }
                                if(i<allLen&&allPermissions[i].id==record.get('id')){
                                    isAllExist=true;
                                }
                                if(isExist&&isAllExist){
                                    break;
                            ...
                            if(!isExist && isAllExist){
                                record.isParent=true;
                                return "<div><span style='float:left'>"+v+"&emsp;&emsp;&emsp;</span><span style='color:#0000FF;float:right'>父级权限&emsp;&uarr;&emsp;</span></div>";
                            }
                        }
                    }
                    return v
...
//          icon : "images/icons/application_form_add.png",handler:this.choicePermission,scope:this},{text:"删除权限",cls : "x-btn-text-icon",
//          icon : "images/icons/application_form_delete.png",handler:function(){Disco.Ext.Util.removeGridRows(this.editGrid);},scope:this}]
    ...
    onEdit : function(ret) {
        ...
        this.editGrid.tempCheck.clear();
        if (ret) {
            var items=this.grid.getSelectionModel().getSelected().get("permissions")
            this.editGrid.store.removeAll();
            if(items){
...
/*      var s = "";
        for (var i = 0; i < this.editGrid.store.getCount(); i++) {
            var r = this.editGrid.store.getAt(i);
            s += r.get("id") + ",";
        }*/
        this.fp.form.baseParams={permissions:this.editGrid.tempCheck.keys.join(',')};
    ...
    onSave:function(){//保存过后
               ...
               /* var reloadTree=this.reloadTree.createDelegate(this);
                Global.systemRoleLoader.remoteObject.load(function(){
                    reloadTree();
    ...
    loadSystemMenu:function(node,e){
        if(node.attributes.id=='root'){
            ...
            this.editGrid.getStore().removeAll();
            this.editGrid.getStore().baseParams={systemMenuId:node.attributes.id}
    ...
    createSysMenuTree:function(){
        this.sysMenuTree = new Ext.tree.TreePanel({
            ...
            rootVisible:false,
            width : 200,        
            tools:[{id:"refresh",handler:function(){this.sysMenuTree.root.reload();},scope:this}],
            root : new Ext.tree.AsyncTreeNode({
                id : "root",
                text : "所有菜单",
                icon : "images/system/root.gif",
    ...
    storeMapping : ["id", "name", "title", "description", "allPermissions", "permissions","users","status","parent"],
    reCalSystemRole:function(){
        ...
        if(!record){
            ...
            Ext.Ajax.request({url:"securityHelper.java",params:{cmd:"reCalSystemRole",id:record.get("id")},scope:this,success:function(req){
    ...
    initComponent : function() {
        ...
        /*this.gridButtons = [new Ext.Toolbar.Separator(),{text:"重新计算角色项",scope:this,handler:this.reCalSystemRole}];*/
...
// 系统角色管理
RoleManagePanel = Ext.extendX(Disco.Ext.TreeCascadePanel,function(superclass){
	return {
		queryParam:'parentId',
		treeCfg : {
			title :"角色树",
