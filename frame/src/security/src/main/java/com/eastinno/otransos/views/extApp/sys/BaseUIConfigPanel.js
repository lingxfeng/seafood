...
if(!Global.baseUIConfigLoader){
    Global.baseUIConfigLoader = new Disco.Ext.MemoryTreeLoader({
        varName:"Global.BASEUICONFIG_NODES",
        url : "baseUIConfig.java?cmd=getTree&pageSize=-1&treeData=true&all=true",
        listeners : {
...
BaseUIConfigListPanel=Ext.extend(Disco.Ext.CrudListPanel,{
    id:"baseUIConfigListPanel",
    //title:"Season管理",
    pageSize:20,
    entityClzName:"com.eastinno.otransos.core.domain.BaseUIConfig",
    baseUrl:"baseUIConfig.java",
    ...
    checkTitle:function(){
        if(this.fp.findSomeThing("title").getValue()&&!this.fp.findSomeThing("id").getValue()){
            Ext.Ajax.request({url:this.baseUrl+"?cmd=checkTitle",params:{title:this.fp.findSomeThing("title").getValue()},success :function(response){
    ...
    this.cm=new Ext.grid.ColumnModel([
        ...
        {header: "模块类", sortable:true,width: 100, dataIndex:"appClass"},
        {header: "父级", sortable:true,width: 100, dataIndex:"parent",renderer:this.objectRender("title")},
        {header: "脚本地址", sortable:true,width: 150, dataIndex:"scripts"}
        ]);
...
BaseUIConfigPanel = function() {
    ...
    this.tree.on("click", function(node, eventObject) {
        ...
        this.list.parent =id?{id:id,title:node.text}:null;
