package com.eastinno.otransos.payment.union.app.service;

import java.util.Map;

import com.eastinno.otransos.payment.union.app.conf.UnionConfig;
import com.eastinno.otransos.payment.union.app.util.HttpUtil;
import com.eastinno.otransos.payment.union.app.util.UpmpCore;
...
public class UnionService{
	...
	public static boolean trade(Map<String, String> req, Map<String, String> resp) {
		...
		String respString = HttpUtil.post(UnionConfig.TRADE_URL, nvp);
	...
	public static boolean query(Map<String, String> req, Map<String, String> resp){
		String nvp = buildReq(req);
		String respString = HttpUtil.post(UnionConfig.QUERY_URL, nvp);
		return verifyResponse(respString, resp);
	}
	...
	public static String buildReq(Map<String, String> req) {
		...
		// 签名结果与签名方式加入请求提交参数组中
		filteredReq.put(UnionConfig.SIGNATURE, signature);
		filteredReq.put(UnionConfig.SIGN_METHOD, UnionConfig.SIGN_TYPE);
    ...
    public static boolean verifySignature(Map<String, String> para) {
        String respSignature = para.get(UnionConfig.SIGNATURE);
        // 除去数组中的空值和签名参数
