...
public class UpmpCore {
    ...
    public static Map<String, String> paraFilter(Map<String, String> para) {
        ...
        for (String key : para.keySet()) {
            ...
            if (value == null || value.equals("") || key.equalsIgnoreCase(UnionConfig.SIGNATURE)
                || key.equalsIgnoreCase(UnionConfig.SIGN_METHOD)) {
                continue;
            }
    ...
    public static String buildSignature(Map<String, String> req) {
		...
		prestr = prestr + QSTRING_SPLIT + UpmpMd5Encrypt.md5(UnionConfig.SECURITY_KEY);
    ...
    public static String createLinkString(Map<String, String> para, boolean sort, boolean encode) {
        ...
        for (int i = 0; i < keys.size(); i++) {
            ...
            if (encode) {
				try {
					value = URLEncoder.encode(value, UnionConfig.CHARSET);
	...
	private static void putKeyValueToMap(StringBuilder temp, boolean isKey,
		...
		if (isKey) {
			...
			map.put(key, URLDecoder.decode(temp.toString(), UnionConfig.CHARSET));
		}
	}
