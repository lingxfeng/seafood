...
public class WxShopMemberCenterAction extends WxShopBaseAction {	
	...
	public Page doToOrder(WebForm form){
    	...
    	qo.setPageSize(-1);
		List<ShopOrderInfo> orderList = (List<ShopOrderInfo>) this.shopOrderInfoService.getShopOrderInfoBy(qo).getResult();		
...
//    	}
	...
	public Page doToAddressList(WebForm form){
        ...
        form.addResult("url", url);
		return new Page("/bcd/wxshop/member/addressList.html");
	}
    ...
    public Page doToApplyWithdrawingCashList(WebForm form){
    	...
    	qo.setOrderType("desc");
    	qo.setLimit(-1);
    	List<ApplyWithdrawCash> applyWithdrawCashList=this.applyWithdrawCashService.getApplyWithdrawCashBy(qo).getResult();
