...
public interface IFieldQueryObject {
     ...
     * 添加要查询的属性
     * 
     * @param fields 要查询的属性列表，比如addField("name","address.city");就会把这些属性做为查询的属性，配合传入的查询对象，生成的查询语句就像select
     *            user.name,user.address.city where...
     */
    IFieldQueryObject addField(String... fields);

    /**
     * 批量往查询对象中添加查询选项，可以是一个完整的具体查询语句，如title='111' and status>0，也可以是包括?号的语句，如title=? and status>0 <code>
    ...
    /**
     * 添加排序规则，比如addOrder("name",OrderType.DESC);则查询的语句为select .... order by name desc; 可以多次添加排序规则，按照添加的顺序排列，
     * 比如addOrder("name",OrderType.DESC);addOrder("born",OrderType.ASC) 则生成的排序规则为select ... order by name desc,born asc;
     * 
     * @param field 要排序的属性
     * @param orderType 排列的类型
     * @return
     */
