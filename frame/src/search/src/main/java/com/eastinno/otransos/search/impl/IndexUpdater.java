package com.eastinno.otransos.search.impl;
...
import com.eastinno.otransos.search.IndexTask;
import com.eastinno.otransos.search.IndexTasker;
import com.eastinno.otransos.search.Searchable;

/**
 * 索引增量更新 该类接受两个参数 idx_path 索引根目录 tasker 任务获取实现类 对应 build.xml 的配置 <target depends="lucene_init" name="lucene_build">
 * <echo message="Build lucene index of ${ant.project.name}"/> <java classname="net.oschina.search.IndexUpdater"
 * classpathref="oschina.classpath" fork="true"> <jvmarg value="-Xmx512m" /> <arg value="-p" /> <arg
 * value="${lucene.dir}" /> <arg value="-t" /> <arg value="net.oschina.search.IndexTasker" /> </java> </target>
 * 
