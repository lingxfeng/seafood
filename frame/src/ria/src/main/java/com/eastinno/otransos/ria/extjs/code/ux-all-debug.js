...
Ext.ux.tree.TreeEditGrid = Ext.extend(Ext.ux.tree.TreeGrid,{
	colCls:'td.x-treegrid-col',
	updateCls:'x-grid3-dirty-cell',
	onClick:function(node,e){
		var el  = e.getTarget(this.colCls);
		var index = el.cellIndex,nid = node.id;
		var ed = this.getCellEditor(index,nid);
		var dataIndex = this.getDataIndex(index);
		var node = this.getNodeById(nid);
		if(ed){
			if(this.fireEvent('beforeedit',this,node,index,dataIndex)!==false){
				ed.currentNode = node;
				ed.col = index;
				ed.dataIndex = dataIndex;
				ed.startEdit(el,this.getNodeAttrval(node,dataIndex)||'');
				this.currentEditor  = ed;
			}
		} 
	},
	valueToString:function(v,startValue){
		if(v && startValue && Ext.isObject(v) && Ext.isObject(startValue)){
			if(v.toString === toString && startValue.toString!==toString){
				v.toString = startValue.toString;
			}else{
				startValue.toString = v.toString;
			}
		}
	},
	onEditComplete:function(ed, v, startValue){
		...
		var col = ed.col;
		this.valueToString(v, startValue);
		if(String(v) !== String(startValue)){
			var nodeColEl = Ext.fly(node.getUI().getTextEl()).parent(this.colCls);
			node.attributes[ed.dataIndex] = v;
			var elNode = node.getUI().elNode, childs = elNode.children;
			var colEl =Ext.fly(childs[col]);
			var tpl = this.getColumnTpl(col);
			var obj = {}; obj[ed.dataIndex] = v;
			if(col==0){
				node.setText(tpl.apply(obj));
			}else{
				colEl.update(tpl.apply(obj));
			}
			colEl.addClass(this.updateCls);
			node.isUpdate = true;
			this.fireEvent('afteredit',node,ed.dataIndex,obj,startValue);
		}
	},
	...
	getNodeAttrval:function(node,dataIndex){
		return Ext.getObjVal(node,"attributes."+dataIndex);
	},
	getDataIndex:function(col){
		var column = this.getColumnByIndex(col);
		if(column)return column.dataIndex;
	},
	...
	findColumn:function(fn,scope){
		var cs = this.columns;
		for (var i = 0; i < cs.length; i++) {
			if(fn.call(scope||window,cs[i],i)){
				return cs[i];
			}
		}
		return null;
	},
	findIndex:function(fn,scope){
		var cs = this.columns;
		for (var i = 0; i < cs.length; i++) {
			if(fn.call(scope||window,cs,i)){
				return i;
			}
		}
		return -1;
	...
	getColumnById:function(id){
		...
		var column = this.findColumn(fn,this);
		return column;
	},
	getColumnTpl:function(col){
		var column = this.getColumnByIndex(col);
		if(column)return column.tpl;
	},
	getColumnByIndex:function(col){
		return this.columns[col];;
	},
	getCellEditor:function(col,nid){
		var column = this.columns[col];
		if(column && column.editable!==false && column.editor){
			if( (column.editor instanceof Ext.Editor)===false){
				column.editor = new Ext.grid.GridEditor(column.editor);
				var ed = column.editor;
				ed.on({
                    scope: this,
                    render: {
                        fn: function(c){
				...
				column.editor.parentEl = this.getContentTarget() ; 
			}
	...
	initComponent:function(){
		this.addEvents(
			...
			/**
			 * @param node 当前节点
			 * @param dataIndex 当前被编辑列的dataIndex
			 * @param value 当前被编辑的value
			 * @param startValue 当前被编辑的前一个value
			 */
			"afteredit"
		);
		this.on('click',this.onClick,this);
		this.on('bodyscroll', this.stopEditing, this);
		Ext.ux.tree.TreeEditGrid.superclass.initComponent.call(this);
	}
});
Ext.reg('ux.treeeditgrid',Ext.ux.tree.TreeEditGrid);
...
Ext.extend(Ext.ux.grid.Search, Ext.util.Observable, {
	...
	,onRender:function() {
		...
		var index = Ext.isNumber(this.prependButtons) ?  this.prependButtons : tb.items.getCount()-1;
		...
		tb.insertButton(index,this.field);
