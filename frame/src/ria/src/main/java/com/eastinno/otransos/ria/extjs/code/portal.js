...
Ext.ux.PortletPanel = Ext.extend(Ext.ux.Portlet,{
	...
	createTools:function(){
		return [
			...
			{id:'maximize',handler:this.onMaximize,hidden:!this.maximize,scope:this},
...
Ext.ux.GridPortletPanel = Ext.extend(Ext.ux.PortletPanel,{
	...
	buildGrid:function(){
		this.store = this.store || new Ext.data.JsonStore({
			...
			autoLoad:(this.url && this.gridAutoLoad)
		});
...
/**
 * 样式选择框
 * Disco.Ext.Util.SelectStyleWin  
 */
Disco.Ext.Util.SelectStyleWin = (function(){
	var win,dataView;
 ...
 * 选择portal的win
 */
Disco.Ext.Util.SelectPortalWin = (function(){
	...
	return {
		show : function(callback , scope ){
			...
			var pc = Disco_RIA.getCfg('portalConfig');
			tempPortals.clear();
			if(pc){
...
Disco.Ext.Portal = function(cfg){
	...
	Disco.Ext.Portal.superclass.constructor.apply(this,arguments);
}
Ext.extend(Disco.Ext.Portal,Ext.ux.Portal,{
	autoScroll:true,
	portalIdPrefix:'',
	defaultType:'portalpanelcolumn',
	defaults:{style:'padding:10px 0 10px 17px'},
	modeCfg:Global.Portal.Models,
	selectStyleWin:Disco.Ext.Util.SelectStyleWin,
	selectPortalWin:Disco.Ext.Util.SelectPortalWin,
	/**
	 * 获得当前模式下,对应的列配置
	 * @return {}
	 */
...
/*	getPortalMode:function(){
	...
	savePortalSeat:function(){
		...
		Ext.Ajax.request({
			scope:this,
			params:params,
			url : 'manage.java?cmd=savePortalSeat' 
		});*/
	...
	updatePortalMode:function(id){
		Ext.Ajax.request({
			url : 'manage.java?cmd=changePortalMode' ,
			params:{portalMode:id},
			scope:this,
			success:function(response,options){
				Disco_RIA.setCfg('portalMode',id);
				this.updatePortalModeHandler(id);
			}
		});
	},
	updatePortalModeHandler : function(id){
		 this.portalMode = id || Disco_RIA.getCfg('portalMode');
		 var pcc = this.buildPortalColumCfg(),items = [],firstColumns = [];
		 var portalCfgs = Disco_RIA.getCfg('portalConfig') || [];
	...
	refreshPortal:function(){
		var portals = Disco_RIA.getCfg('portalConfig');
	...
	selecPortalHandler:function(portals){
		this.getTopToolbar().items.key('saveMode').enable();
		Disco_RIA.setCfg('portalConfig',portals.items);
		this.refreshPortal();
		/*
		Ext.Ajax.request({
			...
			success:function(){
				Ext.apply(Disco_RIA.getCfg(),{portals : portals});
				this.refreshPortal();
			}
		});
		*/
	...
	buildTbar : function(){
		return {
			items:[
				...
				{text : '快捷设置',icon:'/images/icons/bell_link.png',handler:this.onShortCutHanlder,scope:this},
	...
	onShortCutHanlder : function(){
		Disco.Ext.Portal.ShortCutManageWin.show();
	},
	initComponent:function(){
		this.items = this.buildPortalColumCfg();
		this.tbar = this.buildTbar();
		this.on('drop',this.onDrop,this);
		Disco.Ext.Portal.superclass.initComponent.call(this);
	}
});
Ext.reg('ux.portal',Disco.Ext.Portal);
/*
 * *********************************************************************************
 * ******************************自定义portal*****************************************
 * *********************************************************************************
...
var ProjectMilestonesGrid = Ext.extend(Ext.ux.GridPortletPanel,{
	...
	loadSettingUrl:'data/portal/portalConf.json',
...
var BrowserPic = Ext.extend(Ext.ux.PortletPanel,{
	...
	createContent : function(){
		return new Disco.Ext.chart.PicChart({
			chartTitle : '浏览器使用比例',
			store : this.getStore()
		});
	}
...
var RiskCharts = Ext.extend(Ext.ux.ImgPortletPanel,{title:'浏览器使用',url:'portalDemo.java?cmd=getPortalReport&img=browser.jpg'});

Global.Portal.PanelMC.add('RiskCharts',RiskCharts);

var WorkloadCharts = Ext.extend(Ext.ux.ImgPortletPanel,{title:'工作量统计图',url:'portalDemo.java?cmd=getPortalReport&img=baobiao2.jpg'});
Global.Portal.PanelMC.add('WorkloadCharts',WorkloadCharts);


Disco.loadJs("/plugins/disco-ajax/plugins/ShortCutPanel.js",false);

var DiscoIntro = Ext.extendX(Ext.ux.PortletPanel,function(superclass){
	return {
		title : 'DiscoEDP2.0简介',
		buildContent : function(){
			return {
				...
				html  :  "<p style='text-align:center;color:red;font-size:20px'>增强用户体验 提高开发效率 降低开发成本 <br/><hr/></p>"+
                   "<p style='text-indent:24px;'>  Disco企业级应用快速开发平台(DiscoEDP)是由Disco技术研发团队自主研发的一款企业应用快速开发平台，平台通过整合主流的开源框架及技术，提供一套从后端数据整合到前端界面展示为一体的企业应用开发解决方案，让用户轻松实现各种企业应用的快速开发，降低开发成本及风险，提高效益。平台中提供了包括常用界面控件、基础应用软件骨架、快速代码生成工具及各种典型应用抽象等。<br/>　　Disco企业级应用快速开发平台(DiscoEDP)后端以EJS(Disco+JPA+Spring)构架为核心，前端以Ajax、ExtJS及DiscoRIA框架为主体。在实现企业应用快速开发的同时，强大的前端UI功能可以为应用提高用户体验。<br/>　　平台涉及到的主要技术：Spring2.5及以上、JPA1.0及以上、   Hibernate 3.2及以上、 Disco 1.2及以上、ExtJS2.2及以上、DiscoRIA 1.0及以上</p>" +
                        "<p style='text-align:right;'><a style='margin-right:5px;color:#0000ff;' target='_blank' href='http://www.disco.org.com/eastinno/otransos-edp.html'>更多信息...</a></p>"
			}
		},
		createContent : function(){
			return {
...
Global.Portal.PanelMC.add('DiscoIntro',DiscoIntro);
