...
public class ChangeBeanTemplateProcess implements TemplateProcess {
    ...
    private String defaultBeanPackage = "com.disco.business";
    ...
    public ChangeBeanTemplateProcess(String tableName) {
        ...
        this.fileName = javaDir + "/" + defaultBeanPackage.replaceAll("\\.", "/") + "/" + tableName + ".java";
    }
    ...
    public void process(Context context) throws Exception {
        ...
        context.put("TabelIdGenerator", "com.disco.dbo.RandomIdGenerator");// 此处以后改为从参数中读取
        ...
        try {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (parameterTypes.length == 1) {
                    ...
                    boolean isBeanStyle = methodName.length() >= 4 && methodName.startsWith("set")
        ...
        if (setterNames.size() != 0) {
            for (Iterator it = setterNames.iterator(); it.hasNext();) {
                ...
                for (Iterator alls = allProp.iterator(); alls.hasNext();) {
                    ...
                    if (((String) property.get("name")).toLowerCase().equals(propertyName)) {
    ...
    private String readInRemain() {
        ...
        try {
            ...
            while ((s = in.readLine()) != null) {
                if (s.indexOf("package") < 0 && s.indexOf("import") < 0 && s.indexOf("public class") < 0) {
                    remains.append(s);
                    remains.append("\n");
                }
            }
        ...
        String ret = remains.toString().substring(0, remains.toString().lastIndexOf("}"));
        return ret;
    }
