...
import com.eastinno.otransos.dbo.util.Assert;
import com.eastinno.otransos.dbo.util.ClassUtils;
import com.eastinno.otransos.dbo.util.JdkVersion;
import com.eastinno.otransos.dbo.util.StringUtils;

/**
 * 根据注册的类型，负责进行数据类型转换工作
 * 
 * @Author <a href="mailto:ksmwly@gmail.com">lengyu</a>
 * @Creation date: 2011年12月17日 下午3:15:46
 * @Intro
 */
class TypeConverter {
     ...
     * @param requiredType the type we must convert to (or <code>null</code> if not known, for example in case of a
     *            collection element)
     * @param methodParam the method parameter that is the target of the conversion (may be <code>null</code>)
    ...
    public Object convertIfNecessary(Object oldValue, Object newValue, PropertyDescriptor descriptor) throws IllegalArgumentException {
        ...
        return convertIfNecessary(descriptor.getName(), oldValue, newValue, descriptor.getPropertyType(), descriptor, new MethodParameter(
    ...
    private String buildIndexedPropertyName(String propertyName, int index) {
        return (propertyName != null ? propertyName + PropertyAccessor.PROPERTY_KEY_PREFIX + index + PropertyAccessor.PROPERTY_KEY_SUFFIX
                : null);
    }

    private String buildKeyedPropertyName(String propertyName, Object key) {
        return (propertyName != null ? propertyName + PropertyAccessor.PROPERTY_KEY_PREFIX + key + PropertyAccessor.PROPERTY_KEY_SUFFIX
                : null);
    }
