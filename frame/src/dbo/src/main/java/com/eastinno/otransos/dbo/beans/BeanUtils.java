...
package com.eastinno.otransos.dbo.beans;
...
import com.eastinno.otransos.dbo.beans.exception.BeansException;
import com.eastinno.otransos.dbo.util.*;
...
public abstract class BeanUtils {
    ...
    public static <T> T convertType(Object v, Class<T> requiredType) {
        return (T) converter.convertIfNecessary(v, requiredType, (MethodParameter) null);
    }
    ...
    public static Object instantiateClass(Constructor<?> contor, Object[] args) throws BeansException {
        ...
        try {
            if (!Modifier.isPublic(contor.getModifiers()) || !Modifier.isPublic(contor.getDeclaringClass().getModifiers())) {
    ...
    /**
     * Find a method with the given method name and minimal parameters (best case: none), declared on the given class or
     * one of its superclasses. Prefers public methods, but will return a protected, package access, or private method
     * too.
     * <p>
     * Checks <code>Class.getMethods</code> first, falling back to <code>findDeclaredMethodWithMinimalParameters</code>.
     * This allows to find public methods without issues even in environments with restricted Java security settings.
     * 
     * @param clazz the class to check
     * @param methodName the name of the method to find
     * @return the Method object, or <code>null</code> if not found
     * @throws IllegalArgumentException if methods of the given name were found but could not be resolved to a unique
     *             method with minimal parameters
    ...
    /**
     * Find a method with the given method name and minimal parameters (best case: none), declared on the given class or
     * one of its superclasses. Will return a public, protected, package access, or private method.
     * <p>
    ...
    /**
     * Parse a method signature in the form <code>methodName[([arg_list])]</code>, where <code>arg_list</code> is an
     * optional, comma-separated list of fully-qualified type names, and attempts to resolve that signature against the
     * supplied <code>Class</code>.
     * <p>
     * When not supplying an argument list (<code>methodName</code>) the method whose name matches and has the least
     * number of parameters will be returned. When supplying an argument type list, only the method whose name and
     * argument types match will be returned.
     * <p>
     * Note then that <code>methodName</code> and <code>methodName()</code> are <strong>not</strong> resolved in the
     * same way. The signature <code>methodName</code> means the method called <code>methodName</code> with the least
     * number of arguments, whereas <code>methodName()</code> means the method called <code>methodName</code> with
     * exactly 0 arguments.
     * <p>
        ...
        if (firstParen > -1 && lastParen == -1) {
            ...
            for (int i = 0; i < parameterTypeNames.length; i++) {
                ...
                try {
                    ...
                    throw new IllegalArgumentException("Invalid method signature: unable to locate type [" + parameterTypeName + "] for argument "
                            + i);
                }
            }
    ...
    /**
     * Find a JavaBeans <code>PropertyDescriptor</code> for the given method, with the method either being the read
     * method or the write method for that bean property.
     * 
    ...
    public static String canonicalPropertyName(String propertyName) {
        ...
        while (searchIndex != -1) {
            ...
            searchIndex = -1;
            if (keyStart != -1) {
                int keyEnd = buf.toString().indexOf(PropertyAccessor.PROPERTY_KEY_SUFFIX, keyStart + PropertyAccessor.PROPERTY_KEY_PREFIX.length());
                if (keyEnd != -1) {
    ...
    /**
     * Check if the given class represents a "simple" property, i.e. a primitive, a String, a Class, or a corresponding
     * array. Used to determine properties to check for a "simple" dependency-check.
     * 
    ...
    public static boolean isSimpleProperty(Class<?> clazz) {
        ...
        return clazz.isPrimitive() || ClassUtils.isPrimitiveArray(clazz) || ClassUtils.isPrimitiveWrapper(clazz)
                || ClassUtils.isPrimitiveWrapperArray(clazz) || clazz.equals(String.class) || clazz.equals(String[].class)
