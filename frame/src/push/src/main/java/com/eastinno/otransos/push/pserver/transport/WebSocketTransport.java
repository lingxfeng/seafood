...
import com.eastinno.otransos.push.pserver.service.Transports;
...
public class WebSocketTransport extends ITransport {
    ...
    @Override
    protected GenericIO doNewI0Client(ChannelHandlerContext ctx, HttpRequest req, String sessionId) {
    ...
    /*
     * (non-Javadoc)
     * @see com.yongboy.socketio.server.transport.ITransport#doPrepareAction(com.
    ...
    public void doHandle(ChannelHandlerContext ctx, HttpRequest req, MessageEvent e) {
        ...
        WebSocketServerHandshakerFactory wsFactory = new WebSocketServerHandshakerFactory(this.getTargetLocation(req,
     ...
     * @param sessionId
     */
    private void doPrepareClient(ChannelHandlerContext ctx, HttpRequest req, String sessionId) {
        GenericIO client = null;
        try {
            client = (GenericIO) SocketIOManager.getDefaultStore().get(sessionId);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    /*
     * (non-Javadoc)
     * @see com.yongboy.socketio.client.ITransport#doHandle(org.jboss.netty.channel .ChannelHandlerContext,
     * org.jboss.netty.handler.codec.http.websocketx.WebSocketFrame, org.jboss.netty.channel.MessageEvent)
     */
    @Override
    ...
    protected String getSessionId(HttpRequest req) {
        ...
        log.debug("webSocketUrl sessionid " + webSocketUrl.substring(webSocketUrl.lastIndexOf('/') + 1));
    ...
    private String getTargetLocation(HttpRequest req, String sessionId) {
        return "ws://" + req.getHeader(HttpHeaders.Names.HOST) + "/socket.io/1/" + getId() + "/" + sessionId;
    }
}