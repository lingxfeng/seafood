...
import com.eastinno.otransos.push.pserver.transport.ITransport;

/**
 * @author maowei
 * @createDate 2013-10-17下午4:48:41
 */
public class SocketIOTransportAdapter extends SimpleChannelUpstreamHandler {
    private static final Logger log = Logger.getLogger(SocketIOTransportAdapter.class);
    ...
    /**
     * 当本地客户机已经成功的与远程主机建立连接（connected）时，连接断开的时候触发
     */
    @Override
    public void channelDisconnected(ChannelHandlerContext ctx, org.jboss.netty.channel.ChannelStateEvent e)
            throws Exception {
        ...
        if ("websocket,flashsocket,htmlfile".contains(this.currentTransport.getId())) {
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, HttpRequest req, MessageEvent e) throws Exception {
        ...
        sendHttpResponse(ctx, req, SocketIOManager.getInitResponse(req, FORBIDDEN));
    }
    ...
    private void handleHandshake(HttpRequest req, MessageEvent e, String reqURI) {
        ...
        String contentString = String.format(SocketIOManager.getHandshakeResult(), uID);
    ...
    private static String getParameter(QueryStringDecoder queryStringDecoder, String parameterName) {
        if (queryStringDecoder == null || parameterName == null)
            return null;

        List<String> values = queryStringDecoder.getParameters().get(parameterName);
     ...
     * @throws IOException
     */
    private void handleStaticRequest(HttpRequest req, MessageEvent e, String reqURI) throws IOException {
