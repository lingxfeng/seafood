...
import com.eastinno.otransos.push.pserver.service.IOHandler;
...
public abstract class GenericIO extends EventClientIO implements IOClient {
    ...
    public GenericIO(ChannelHandlerContext ctx, HttpRequest req, String uID, String namespace) {
        this(ctx, req, uID);
    ...
    public void heartbeat(final IOHandler handler) {
        // 以下为示范代码
        /*
         * prepareHearbeat(); // add new task // 定时设置client为不可用 scheduleClearTask(handler); sendEncoded("2::");
         */
    }

    protected void scheduleClearTask(final IOHandler handler) {
        scheduledFuture = SocketIOManager.scheduleClearTask(new ClearTaskSpeed(getSessionID(), handler),
                SocketIOManager.option.heartbeat_timeout, TimeUnit.SECONDS);
    }

    public void scheduleRemoveTask(final IOHandler handler) {
        scheduledFuture = SocketIOManager.scheduleClearTask(new ClearTaskSpeed(getSessionID(), handler), 1L,
    ...
    public boolean equals(Object obj) {
        ...
        return this.getId().equals(objClient.getId()) && this.getSessionID().equals(objClient.getSessionID());
    }
}