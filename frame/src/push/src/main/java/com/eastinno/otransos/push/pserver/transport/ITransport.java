package com.eastinno.otransos.push.pserver.transport;
...
import com.eastinno.otransos.push.pserver.service.SocketIOManager;
import com.eastinno.otransos.push.pserver.service.Store;
...
public abstract class ITransport {
     ...
     * @return
     */
    protected abstract GenericIO doNewI0Client(ChannelHandlerContext ctx, HttpRequest req, String sessionId);
     ...
     * @param e
     */
    public void doHandle(ChannelHandlerContext ctx, WebSocketFrame frame, MessageEvent e) {
        // TO DO NOTHING ...
    }
     ...
     * @param e
     */
    public void doHandle(ChannelHandlerContext ctx, HttpRequest req, MessageEvent e) {
        ...
        resp.setContent(ChannelBuffers.copiedBuffer(respContent, CharsetUtil.UTF_8));
    ...
    private void handleInvalidRequest(HttpRequest req, MessageEvent e) {
        ...
        // req.getUri());
        HttpResponse resp = SocketIOManager.getInitResponse(req, HttpResponseStatus.FORBIDDEN);
     ...
     * @return
     */
    protected GenericIO initGenericClient(ChannelHandlerContext ctx, HttpRequest req) {
        String sessionId = getSessionId(req);
        ...
        if (client == null) {
            log.debug("initGenericClient the client is null with id : " + sessionId);
            client = doNewI0Client(ctx, req, sessionId);
    ...
    protected String handleContent(GenericIO client, String content) {
        ...
        if (content.startsWith("d=")) {
            ...
            QueryStringDecoder queryStringDecoder = new QueryStringDecoder(content, CharsetUtil.UTF_8, false, 2);

            // TODO 这里当遇到 & 符号时，将无法正常解析; 有待修补
            // fixit it: http://www.x2x1.com/show/8659522.aspx
            /**
             * HttpRequest request = (HttpRequest) e.getMessage(); HttpPostRequestDecoder decoder = new
             * HttpPostRequestDecoder(new DefaultHttpDataFactory(false), request); InterfaceHttpData data =
             * decoder.getBodyHttpData("fromField1"); if (data.getHttpDataType() == HttpDataType.Attribute) { Attribute
             * attribute = (Attribute) data; String value = attribute.getValue() System.out.println("fromField1 :" +
             * value); }
             */
     ...
     * @param res
     */
    protected void sendHttpResponse(ChannelHandlerContext ctx, HttpRequest req, HttpResponse res) {
