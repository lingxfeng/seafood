...
import com.eastinno.otransos.web.ajax.AjaxConfigManager;
 ...
 * </p>
 * <p>
 * Description:调用配置信息处理类,处理并存放disco-web.xml中的配置信息
 * </p>
 * <p>
...
public class DefaultWebConfig implements WebConfig {
    ...
    // 所有错误处理程序
    private final List errorHandler = new ArrayList();
    ...
    private List importsConfigures = new ArrayList();
    ...
    public DefaultWebConfig() {
        configManagers.add(AjaxConfigManager.getInstance());
    }
    ...
    public synchronized void init() {
        ...
        try {
            ...
            for (int i = 0; i < beanDefinitions.size(); i++) {
                ...
                if (ExceptionInterceptor.class.isAssignableFrom(bd.getBeanClass()))
    ...
    public void parseConfig(InputStream in) throws Exception {
        BufferedInputStream is = new BufferedInputStream(in);
        IConfigFactory icf = new XMLConfigFactory(is);
        Map map = new HashMap();
        map = icf.initOtherCfg();// 其它配置信息,除了form，module及page以外的其它配置信息
        if (map != null) {
            // 若设置了模版根目录则使用模版根目录路径
            if (map.get(ConstantUtil.TEMPLATE_BASE_PATH) != null)
                templateBasePath = (String) map.get(ConstantUtil.TEMPLATE_BASE_PATH);
            // 是否调试模式
            if (map.get(IConfigFactory.DEBUG) != null) {
                isDebug = Boolean.valueOf((String) map.get(IConfigFactory.DEBUG)).booleanValue();
            }
            // 文件上传大小
            if (map.get(IConfigFactory.MaxUploadFileSize) != null) {
                int maxSize = Integer.valueOf((String) map.get(IConfigFactory.MaxUploadFileSize)).intValue() * 1024;
                if (maxSize > 0)
                    maxUploadFileSize = maxSize;
            }
            // 上传文件缓存大小
            if (map.get(IConfigFactory.UploadSizeThreshold) != null) {
                int maxSize = Integer.valueOf((String) map.get(IConfigFactory.UploadSizeThreshold)).intValue() * 1024;
                if (maxSize > 0)
                    uploadSizeThreshold = maxSize;
            }
            // 最大直接跳转Action次数
            if (map.get(IConfigFactory.MaxDirectJumpToActionTimes) != null) {
                int maxTimes = Integer.valueOf((String) map.get(IConfigFactory.MaxDirectJumpToActionTimes)).intValue();
                if (maxTimes >= 0)
                    maxDirectJumpToActionTimes = maxTimes;
            }
            // 是否自动检查权限
            if (map.get(IConfigFactory.PermissionVerify) != null) {
                permissionVerify = Boolean.valueOf((String) map.get(IConfigFactory.PermissionVerify)).booleanValue();
            }
            /**
            ...
            /**
             * 设置全局变量Globals.Language的值为指定的语言，这样就可以使得Disco后台输出指定语言的日志
             */
            ...
            if (map.get(IConfigFactory.DefaultActionPackage) != null) {
                String packages = (String) map.get(IConfigFactory.DefaultActionPackage);
        ...
        Iterator ocs = configManagers.iterator();
        while (ocs.hasNext()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (StringUtils.hasLength(classname)) {
                    ...
                    if (StringUtils.hasLength(destroymethod)) {
                        Method destroy_method = obj.getClass().getMethod(destroymethod);
