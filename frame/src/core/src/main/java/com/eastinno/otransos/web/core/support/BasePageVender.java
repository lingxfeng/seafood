...
abstract public class BasePageVender extends AbstractPageVender {
    ...
    public boolean venderPage(HttpServletRequest request, HttpServletResponse response, Page page, WebInvocationParam param) {
        ...
        try {
            ...
            if (template != null) {
                ...
                context.put("session", PageVenderSupport.session2map(request.getSession()));
                ...
                if (FrameworkEngine.getValidateManager() != null)
                    context.put("errors", FrameworkEngine.getValidateManager().getErrors().getErrors());

                // 把ActionContext中的数据也放置到结果集中，因此也可以通过ActionContext来往客户端发传递参数
                Iterator it = ActionContext.getContext().getContextMap().entrySet().iterator();
                while (it.hasNext()) {
                ...
                return mergeTemplate(template, context, response, page, request, param);
            } else {
            ...
            throw new FrameworkException(I18n.getLocaleMessage("core.web.not.find.tempate")
                    + FrameworkEngine.getWebConfig().getTemplateBasePath() + uri, rnfe);
            ...
            throw new FrameworkException(I18n.getLocaleMessage("core.web.in.the.template.file.syntax.error.not.the.normal.analytical")
                    + FrameworkEngine.getWebConfig().getTemplateBasePath() + uri, pee);
     ...
     * 
     * @param template 处理完成的Velocity模板对象
     * @param context 处理完成的Velocity上下文对象
     * @param response HttpServletResponse对象
     * @param page 处理完成的Page对象
     * @return
     */
    abstract protected boolean mergeTemplate(Template template, Context context, HttpServletResponse response, Page page,
     ...
     * @throws Exception
     */
    protected Template getTemplate(String templateName, String encoding) throws ResourceNotFoundException, ParseErrorException,
        ...
        if (template == null || (f.exists() && f.lastModified() > template.getLastModified()))
