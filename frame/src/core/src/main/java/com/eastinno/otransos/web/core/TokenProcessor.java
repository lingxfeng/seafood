...
public class TokenProcessor {
    ...
    /**
     * 检测令牌是否合法，并清空令牌值。判断令牌是否合法 如果Session中保存有令牌并且在提交的请求中令牌和Session中的令牌相等的话，返回true 如果Request中没有Session对象，返回false
     * 如果session中没有token，返回false 如果session中的token和请求中的token不一致，返回false
     * 
     ...
     * 
     * @param request
     * @param reset 是否在验证token后清空Session中的token
     * @return
     */
    ...
    public synchronized void saveToken() {
        ...
        if (token != null) {
            ActionContext.getContext().getSession().setAttribute(Globals.TOKEN_NAME, token);
        }
    }
