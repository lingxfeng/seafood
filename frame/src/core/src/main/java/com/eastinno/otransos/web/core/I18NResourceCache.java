...
public class I18NResourceCache {
    ...
    /**
     * 根据一个国际化模块的名字和一个语言名字来返回一个确定的属性集对象。 如果国际化模块的名字为空，则使用保存在当前线程中的默认国际化模块的名字，如果模块的名字不为空，则设置当前线程默认的模块名字为该值。
     * 如果语言名字为空，则使用默认的语言集（从Action中赋值），如果不为空，则根据该语言返回属性集对象，不改变默认语言集。
     * 
     * @param pageName 国际化模块的名字，如cms模块的所有页面可以使用一个属性文件来保存需要国际化字段的值，而blog模块可以使用一个单独的属性文件来保存需要国际化字段的值。
     * @param localLanguage 语言名字，该值为Locale.getLanguage()方法返回的值。
     * @return 返回一个属性集对象。
     */
    ...
    public ResourceBundle getApplicationResourceBundle(Locale locale) {
        return java.util.ResourceBundle.getBundle(Globals.RESOURCE_FILE_PATH + Globals.APPLICATION_PROPERTIES_PREFIX,
                locale);
    }
    ...
    private ResourceBundle getResourceBundleFromClassPath(Locale locale) {
        ...
        bundle = java.util.ResourceBundle.getBundle(basePath + propertys, locale);
        return bundle;
    }
    ...
    private ResourceBundle getResourceBundleFromFile(Locale locale) {
        String propertys = resourceName.get();
        java.util.ResourceBundle bundle = null;
        String ext = "xml".equalsIgnoreCase(Globals.PROPERTIES_TYPE) ? ".xml" : ".properties";
        String localePrefix = I18n.getLocalePrefix(locale);
        String fileName = "WEB-INF/" + basePath + propertys + "_" + localePrefix + ext;
        String originProperties = "WEB-INF/" + basePath + propertys + ext;
        java.io.File dir = new java.io.File(Globals.APP_BASE_DIR);
        try {
            File file = new File(dir, fileName);
            bundle = ".xml".equals(ext) ? new XMLPropertyResourceBundle(new FileInputStream(file))
                    : new PropertyResourceBundle(new FileInputStream(file));
        } catch (java.io.IOException e) {
            try {
                File file1 = new File(dir, originProperties);
                bundle = ".xml".equals(ext) ? new XMLPropertyResourceBundle(new FileInputStream(file1))
                        : new PropertyResourceBundle(new FileInputStream(file1));
            } catch (java.io.IOException e1) {
                throw new MissingResourceException("Missing resource " + originProperties, this.getClass().getName(),
                        "getPropertiesObj");
            }
        }
        return bundle;
    ...
    private class XMLPropertyResourceBundle extends ResourceBundle {
        ...
        public Enumeration getKeys() {
            ...
            return new ResourceBundleEnumeration(lookup.keySet(), resourcebundle == null ? null
