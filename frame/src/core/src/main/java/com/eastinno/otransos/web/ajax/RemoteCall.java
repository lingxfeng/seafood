...
public class RemoteCall {
     ...
     * @throws Exception
     */
    private Method findRemoteMethod(Object service, Class[] clzs) throws Exception {
        java.lang.reflect.Method ret = null;
        if (clzs == null)
            clzs = new Class[0];
        if (AjaxUtil.getServiceContainer().getSignatures(serviceName, methodName) != null) {
            Method[] ms = service.getClass().getMethods();
            for (Method m : ms) {
                if (m.getName().equals(methodName)) {
                    ret = m;
                    for (int i = 0; i < m.getParameterTypes().length && i < clzs.length; i++) {
                        if (!m.getParameterTypes()[i].isAssignableFrom(clzs[i]))
                            ret = null;
                    }
                }
            }
        ...
        if (ret == null)
            throw new ServiceNotAvailableException(serviceName + "." + methodName);
    ...
    public CallParameter[] convertJsonParamater(Map map) {
        ...
        while (it.hasNext()) {
            ...
            if (name.length() > 10 && name.substring(0, 11).equals("ajax-call-p")) {
                String index = name.substring(11, name.indexOf('.', 11));
                addProperty(propertys, Integer.parseInt(index), name.substring(name.indexOf('.', 11) + 1),
                        map.get(name));
            }
        }
        ...
        java.util.Collections.sort(ps, new Comparator() {
            public int compare(Object arg0, Object arg1) {
                return ((CallParameter) arg0).getIndex() - ((CallParameter) arg1).getIndex();
            }
        });
    ...
    public void convertCallParameter2Obj(Class[] clzs, Object[] objs, CallParameter[] ps) {
        for (int i = 0; i < ps.length; i++) {
            CallParameter parameter = ps[i];
            clzs[i] = ps[i].getType();
            if (ClassUtils.isPrimitiveOrWrapper(clzs[i]) || clzs[i] == String.class) {
                if (parameter.getPropetys().get("value") != null)
                    objs[i] = JSonConvertUtil.simpleConvert((String) parameter.getPropetys().get("value"), clzs[i]);// ,new
                // Object[]{});
            } else if (Number.class.isAssignableFrom(clzs[i]) || java.util.Date.class.isAssignableFrom(clzs[i])) {
                objs[i] = new BeanWrapperImpl().convertIfNecessary(parameter.getPropetys().get("value"), clzs[i]);// ,new
                // Object[]{});
            } else if (clzs[i].isArray()) {
                // 数据组转换
                objs[i] = JSonConvertUtil.arrayConvert((String) parameter.getPropetys().get("value"), clzs[i]);
            } else if (Map.class.isAssignableFrom(clzs[i])) {
                Map map = new java.util.HashMap();
                java.util.Iterator pvs = parameter.getPropetys().entrySet().iterator();
                while (pvs.hasNext()) {
                ...
                objs[i] = JSonConvertUtil.collectionConvert((String) parameter.getPropetys().get("value"), clzs[i]);
            } else {
                ...
                /*
                 * BeanWrapper wrapper = new BeanWrapper(objs[i]); java.util.Iterator pvs =
                 * parameter.getPropetys().entrySet() .iterator(); while (pvs.hasNext()) { Map.Entry en = (Map.Entry)
                 * pvs.next(); String propertyName = (String) en.getKey(); if (propertyName.indexOf('.') > 0) {
                 * initBeanProperty(objs[i], propertyName); } if (wrapper.isWritableProperty(propertyName))
    ...
    public void addProperty(java.util.Map map, int index, String name, Object value) {
        ...
        if ("java-type".equalsIgnoreCase(name)) {
            try {
                p.setType(ClassUtils.forName((String) value, ClassUtils.getDefaultClassLoader()));
            } catch (Exception e) {
    ...
    public void initBeanProperty(Object bean, String propertyName) {
        ...
        if (wrapper.getPropertyValue(p) == null)
            wrapper.setPropertyValue(p, BeanUtils.instantiateClass(wrapper.getPropertyDescriptor(p).getPropertyType()));
