...
public class DefaultExceptionHandle implements ExceptionInterceptor {
    ...
    private static final Logger logger = Logger.getLogger(DefaultExceptionHandle.class);
    ...
    protected void showErrInfo(Throwable e) {
        try {
            ...
            if (errorPage != null)
                ActionContext.getContext().getResponse().sendRedirect(errorPage);
            else if (FrameworkEngine.getWebConfig().isDebug())
                error(ActionContext.getContext().getRequest(), ActionContext.getContext().getResponse(), e);
            else
                info(ActionContext.getContext().getRequest(), ActionContext.getContext().getResponse(), e);
        } catch (Exception se) {
            se.printStackTrace();
            logger.error(I18n
                    .getLocaleMessage("core.web.Exception.Handling.Framework.deal.with.serious.mistake.please.check.configuration.is.correct"));
        }
    }
     ...
     */
    protected void error(HttpServletRequest request, HttpServletResponse response, Throwable e) throws ServletException {
        ...
        if (why != null && why.trim().length() > 0) {
            html.append(why);
            html.append("\r\n<br>" + I18n.getLocaleMessage("core.web.for.enquiries.please.detail")
                    + "<a href='http://www.disco.org.cn/' target='_blank'>http://www.disco.org.cn</a>\r\n");
        }
        ...
        html.append("	<span>&copy; 2006-" + java.util.Calendar.getInstance().get(Calendar.YEAR) + " Disco "
                + I18n.getLocaleMessage("core.web.simple.open.source.framework.JAVA") + "</span>\r\n");
        html.append("</div>\r\n");
        html.append("</body>\r\n");
        html.append("</html>");
    ...
    protected void info(HttpServletRequest request, HttpServletResponse response, Throwable e) throws ServletException {
         ...
         * cause.printStackTrace(new PrintWriter(sw)); html.append(sw.toString()); html.append("</pre> </div>\r\n");
         */
        html.append("</div>\r\n");
        html.append("<hr class=\"line\" />\r\n");
        html.append("<div class=\"copyright\">\r\n");
        html.append("	<span>&copy; 2006-")
                .append(java.util.Calendar.getInstance().get(Calendar.YEAR))
                .append("　Disco " + I18n.getLocaleMessage("core.web.simple.open.source.framework.JAVA")
                        + "</span>\r\n");
        html.append("</div>\r\n");
        html.append("</body>\r\n");
        html.append("</html>");
