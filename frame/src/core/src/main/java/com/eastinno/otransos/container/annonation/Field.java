 ...
 * 
 * <pre>
 * 该标签是Disco中的一个重要标签，该标签提供的属性信息有以下几个作用：
...
public @interface Field {
    ...
    /**
     * 标识该属性是否属于只写入的 一个属性不具有可写入的性质，则在使用Disco中的实用方法如WebForm.toPo(Object)等方法进行简易赋值的时候，将会忽略这些属性的值
     * 
    ...
    /**
     * 标识该属性是否属于可读的 一个属性若不可读，则在使用Disco框架中的实用方法如WebForm.addPo(Object)等方法把对象中的属性值添加到表示层时，则将会忽略这个属性，从而达到对数据保护的目的。
     * 
