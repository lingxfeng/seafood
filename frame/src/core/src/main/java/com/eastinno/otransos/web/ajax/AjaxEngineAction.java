    ...
    public Page execute(WebForm form, Module module) throws Exception {
        ...
        if ("".equals(command)) {
            ...
            try {
                ...
                throw new Exception(
                        I18n.getLocaleMessage("core.ajax.methods.name.is.not.correct.in.the")
                                + this.getClass()
                                + I18n.getLocaleMessage("core.ajax.no.find")
                                + cmd
                                + I18n.getLocaleMessage("core.ajax.method.Please.check.your.page.for.cmd.parameter.values.are.correct"));
            }
        }
        return forward;
    }
    ...
    public Page doAjaxCall(WebForm form, Module module) throws Exception {
        ...
        try {
            ...
            form.addResult(
                    "result",
                    "return \"数据验证错误:"
                            + CommUtil.convert2unicode(FrameworkEngine.getValidateManager().getErrors().getMessage(),
                                    false) + "\"");
        } catch (Exception e) {
            form.addResult("error", "true");
