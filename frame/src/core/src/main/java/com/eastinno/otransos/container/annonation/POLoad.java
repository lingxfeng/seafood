...
/**
 * 该标签用来加载关联<br>
 * 在Disco框架中提供了一个非常实用的方法把Form中的数据传入模型(域)对象中，toPo过程中，我们可以把前台表单中的普通类型的属性值传入到模型对象中
 * 但对于一些关联对象，我们在表单中只能存放关联对象的id，而在赋值的时候需要通过这个id从业务（或持久）层中加载这个对象，然后设置到指定的属性中，此时可以使用POLoad这个标签，他可以处理这个自动关联。 请看下面的例子：
 * 
 * <pre>
 * public class BBSDir
 * {
 ...
 * }
 * 
 * 在默认的情况下，Disco框架部是使用POLoadDao的实现来加载关联的对象，因此需要我们的容器中有一个POLoadDao的实现Bean。如下面的配置使用JPA的DAO来加载关联对象：
 * 
 * <bean id="jpaPoLoader" class="com.eastinno.otransos.core.dao.impl.JpaPOLoaderImpl">
...
public @interface POLoad {
     ...
     * 
     * @see com.eastinno.otransos.web.tools.POLoadDao.class
