...
/**
 * Disco中的配置信息，框架中所有部件或对象访问系统配置信息都是由该类访问。
 * 
 * @author lengyu
 */
public interface WebConfig {
    ...
    /**
     * 得到初始化程序 也即在启动Disco框架时，自动执行的初始化应用程序。
     * 
     * @return 系统中的初始化程序
     */
    List getInitApps();
    ...
    /**
     * 获取系统中的所有全局拦截器 全局拦截器是针对所有请求都会执行的，拦截器可以是前置拦截BeforeInterceptor，后拦截AfterInterceptor或者是环绕拦截AroundInterception几种
     * 
     * @return 系统中的全局拦截器
     */
    ...
    /**
     * 获取系统中的异常(错误)处理器 Disco通过配置在系统中的这些异常处理器，统一进行所有异常(错误)的拦截处理工作。
     * 
     * @return 所有异常处理器
     */
