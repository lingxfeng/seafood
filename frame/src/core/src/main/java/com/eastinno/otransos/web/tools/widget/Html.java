package com.eastinno.otransos.web.tools.widget;
...
public class Html {
    ...
    public String getToken() {
        String token = (String) ActionContext.getContext().getSession().getAttribute(Globals.TOKEN_NAME);
        if (token == null || "".equals(token)) {
            return null;
        } else {
            return "<input type=\"hidden\" id=\"" + Globals.TOKEN_NAME + "\" name=\"" + Globals.TOKEN_NAME + "\" value=\"" + token
                    + "\" />";
        }
    }
    ...
    public String js(String path) {
        ...
        for (int i = 0; i < s.length; i++) {
            String f = s[i];
            if (!f.endsWith(".js"))
                f = (new StringBuilder(String.valueOf(f))).append(".js").toString();
            s[i] = f;
            ret = (new StringBuilder(String.valueOf(ret))).append("<script type=\"text/javascript\" src=\"").append(this.url(f))
                    .append("\"></script>\r\n").toString();
        }
    ...
    public String css(String path) {
        ...
        for (int i = 0; i < s.length; i++) {
            String f = s[i];
            if (!f.endsWith(".css"))
                f = (new StringBuilder(String.valueOf(f))).append(".css").toString();
            s[i] = f;
            ret = (new StringBuilder(String.valueOf(ret))).append("<link rel=\"stylesheet\" type=\"text/css\" href=\"").append(this.url(f))
                    .append("\" />\r\n").append("<style type=\"text/css\">\r\n").append("\t@import url(\"").append(this.url(f))
                    .append("\");\r\n").append("</style>\r\n").toString();
        }
    ...
    public String extjs() {
        ...
        f.append("<script>Ext.BLANK_IMAGE_URL=\"/plugins/resources/images/default/s.gif\";</script>");
        return f.toString();
    }

    public String tinymce() {
        String f = "";
        f = (new StringBuilder(String.valueOf(f))).append(js(this.url("/javascript/tiny_mce/tiny_mce"))).toString();
        return f;
    }

    public String calendar() {
        String f = "";
        f = (new StringBuilder(String.valueOf(f))).append(css("javascript/jscalendar-1.0/calendar-brown")).toString();
        f = (new StringBuilder(String.valueOf(f))).append(js("javascript/jscalendar-1.0/calendar")).toString();
        f = (new StringBuilder(String.valueOf(f))).append(js("javascript/jscalendar-1.0/lang/cn_utf8")).toString();
        f = (new StringBuilder(String.valueOf(f))).append(js("javascript/calendar/initcalendar")).toString();
        return f;
    }
    ...
    public String showCalendar(String element, String format) {
        ...
        String s = "<img src=\"" + path + "\" border=\"0\" alt=\"请选择\" onclick=\"return showCalendar('" + element + "', '" + format
                + "');\" />";
        return s;
    }

    /**
     * 自动加载disco所需js和css文件到页面
     */
    public String disco() {
        String f = "";
        f = (new StringBuilder(String.valueOf(f))).append(js("/javascript/disco/default")).toString();
        f = (new StringBuilder(String.valueOf(f))).append(css("/stylesheet/disco/default")).toString();
        return f;
    }
        ...
        try {
            ...
            if (context != null) {
                if (context.getWebInvocationParam() != null
                        && !IPathMappingRuler.CLASSIC_PATTERN.equals(context.getWebInvocationParam().getUrlType()))
                    ret = context.getRequest().getContextPath() + "/" + context.getWebInvocationParam().getSuffix() + ret;
                else
    ...
    public String forward(String command) {
        ...
        if (context != null) {
            ...
            if ("true".equals(showHtmlPage)) {
                ...
                if (IPathMappingRuler.CLASSIC_PATTERN.equals(param.getUrlType())) {
                    if (StringUtils.hasText(cmd)) {
                        ret = context.getRequest().getContextPath() + module + "." + param.getSuffix() + "?cmd=" + cmd;
                    } else {
                        ret = context.getRequest().getContextPath() + module + "." + param.getSuffix();
                    }
                } else
                    ret = context.getRequest().getContextPath() + "/" + param.getSuffix() + module + "/" + cmd;
            }
        }
        return ret;
    }
     ...
     * 自动在动/静场景下转换链接地址类型
     * 
     * @param obj 一个实现了AutoChangeLink接口的对象
     * @return
     */
    ...
    public String autoLink(String url) {
        String suffix = "";
        String p = isShowHtmlPage() ? handleModuleCmdHtml(url) : handleModuleCmdUrl(url);
        if (isShowHtmlPage()) {
            p = "/" + p;
            suffix = ".html";
        } else {
    ...
    /*
     * [news.tutorial]对应news.java?cmd=tutorial 对应的静态文件为news/tutorial/xx.htm [news.
     * list/dirSn.type]=news.java?cmd=list&dirSn=WebForm.get(dirSn)&type=WebForm .get(type)
     ...
     * 
     * @param ms URL字符串如：[news.frontList/sn=ly/1.]
     * @return
     */
    ...
    private String showPageHtmlByUrl(int currentPage, int pages, String url) {
        ...
        String p = htmlTool.isShowHtmlPage() ? htmlTool.handleModuleCmdHtml(url) : htmlTool.handleModuleCmdUrl(url);
