...
public @interface FormPO {
     ...
     * 在Disco框架中，可以直接通在模型(域)对象上通过标签来指定只允许注入哪些属性值。如果一个属性值为可注入的，则可以使用WebForm.toPo(obj)方法，把WebForm中与obj属性名称相同的属性值设置到obj中。
     * 需要注入的属性与逗号(,)作为分逗符。
     * 
     * <pre>
     ...
     * 在Disco框架中，一个模型(域)对象的属性默认情况下全部都是可注入的，我们可以通过disInject来指定不可注入的属性。 多个不可注入的属性使用逗号(,)作为分隔。 如上面inject示例，也可以写成下面的形式：
     * 
     * <pre>
    ...
    /**
     * 指定不能直接通过addPO方法把数据添加到前台的属性，对应Field标签的readable=false属性 假如我们不想把bornDate这一个涉及用户隐私的数据显示到前台，则可以使用下面的注解
     * 
     * <pre>
    ...
    /**
     * 验证器,可以在PO中指定完整的验证器。 用来指定模型(域)对象中各属性的验证器，还可以使用复合验证器。 当很多属性的验证规则都一样的时候，通过FormPO中的validators来指定验证器，将会省略很多重复注解信息。
     * 如下面的示例：
     * 
     * <pre>
