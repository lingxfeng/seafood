...
public class ValidatorManager {
     ...
     */
    private Map<String, Class> validatorsType = new HashMap<String, Class>();

    /**
     * 缓存验证目标对象
     */
    private Map<Object, TargetObject> targetObjects = new HashMap<Object, TargetObject>();

    /**
     * 缓存记录验证类型，如对于Person对象，name为person，值为com.eastinno.otransos.demo.domain.Person
     */
    private Map<String, Class<?>> targetTypes = new HashMap<String, Class<?>>();

    /**
	 * 
	 */
    private static ThreadLocal<Map> errorsWrapper;

    /**
     * 存放验证错误结果
     */
    private ThreadLocal<Errors> errors;
     ...
     * 从验证管理器中返回验证器
     * 
     * @param name 验证器的逻辑名称，如string、required、range等
    ...
    public Validator getValidator(String name) {
        ...
        if (v == null)
            throw new FrameworkException(I18n.getLocaleMessage("core.web.in.system.No.registration.called") + name
                    + I18n.getLocaleMessage("core.web.Verification.check"));
        return v;
    }
        ...
        if (vclz != null) {
            try {
                ...
                throw new FrameworkException(
                        I18n.getLocaleMessage("core.web.Unable.to.initialize.type")
                                + vclz
                                + I18n.getLocaleMessage("core.web.Validator.please.confirm.that.the.test.for.whether.the.default.constructor.function"),
                        e);
            }
        } else
     ...
     * @return 系统中的所有验证器类型
     */
    public Collection<Class> getValidatorsType() {
     ...
     * @return 目标验证对象集合
     */
    public Collection<TargetObject> getValidateObjects(ValidateType type) {
        return targetObjects.values();
    }
    ...
    public Errors createValidateMulitErrors(Object obj) {
        if (errorsWrapper == null) {
            errorsWrapper = new ThreadLocal<Map>();
            errorsWrapper.set(new HashMap());
        }
     ...
     * 
     * @param property 要检测的属性
     * @return 若存在指定属性的验证对象信息，则返回该对象，否则返回null
     */
    public TargetObject findValidateObject(PropertyDescriptor property) {
        TargetObject validateObject = null;
        List ret = findValidators1(property);
        Iterator it = ret.iterator();
        while (it.hasNext()) {
            Map map = (Map) it.next();
            Iterator vsi = map.entrySet().iterator();
            while (vsi.hasNext()) {
                Map.Entry en = (Map.Entry) vsi.next();
                List<com.eastinno.otransos.container.annonation.Validator> vds = (List) en.getValue();
                if (vds.size() > 0) {
                    for (int i = 0; i < vds.size(); i++) {
                        if (validateObject == null)
                            validateObject = this.addValidateObject(vds.get(i), property);
                        else
                    ...
                    if (key instanceof Field) {
                        Field f = (Field) key;
                        if (!StringUtils.hasLength(validateObject.getDisplayName()))
                        ...
                        if (validateObject.getFieldName() == null || validateObject.getFieldName().indexOf(',') > 0)
    ...
    /**
     * 通过Validator，Field及FormPO标签中的validators标签中的标签来判断验证对象配置。该方法将返回一个结构奇怪的数据结构，也即List<Map<Annonation,List<Validator>>>。
     * 即把针对字段的Field、Validator标签；针对方法的Field、Validator标签以；针对AFormPO的标签等分别放在存放在返回列表中。
     * 
     * @param property 属性名称
     * @return List<Map<Annonation,List<Validator>>>
     */
    private List findValidators1(PropertyDescriptor property) {
        List ret = new ArrayList();

        Method method = property.getWriteMethod();// 只有setter才需要进入验证
        if (method != null) {

            // 首先查询property对应的Field属性中的Field配置
            try {
                java.lang.reflect.Field f = method.getDeclaringClass().getDeclaredField(property.getName());
                List<com.eastinno.otransos.container.annonation.Validator> vds = new ArrayList<com.eastinno.otransos.container.annonation.Validator>();
                Map<Object, List<com.eastinno.otransos.container.annonation.Validator>> map = new HashMap<Object, List<com.eastinno.otransos.container.annonation.Validator>>();
                // 首先通过Validator标签来判断验证对象
                com.eastinno.otransos.container.annonation.Validator vc = method.getAnnotation(com.eastinno.otransos.container.annonation.Validator.class);
                if (vc != null) {
                    vds.add(vc);
                }
     ...
     * @return 查询结果
     */
    public List<com.eastinno.otransos.container.annonation.Validator> findValidators(PropertyDescriptor property) {
        List<com.eastinno.otransos.container.annonation.Validator> vds = new ArrayList<com.eastinno.otransos.container.annonation.Validator>();
        List ret = findValidators1(property);
        Iterator it = ret.iterator();
        while (it.hasNext()) {
     ...
     * @return 查询结果
     */
    protected List<com.eastinno.otransos.container.annonation.Validator> findValidators(Field field) {
        List<com.eastinno.otransos.container.annonation.Validator> vds = new ArrayList<com.eastinno.otransos.container.annonation.Validator>();
        if (field != null) {
            if (!"NULL".equalsIgnoreCase(field.validator().name())) {
                com.eastinno.otransos.container.annonation.Validator vconfig = field.validator();
                vds.add(vconfig);

            }
            if (field.validators() != null) {
                for (int i = 0; i < field.validators().length; i++) {
                    com.eastinno.otransos.container.annonation.Validator vtor = field.validators()[i];
                    vds.add(vtor);
                }
            }
        }
     ...
     * @return 查询结果
     */
    public List<com.eastinno.otransos.container.annonation.Validator> findValidators(FormPO formPo, String name) {
        List<com.eastinno.otransos.container.annonation.Validator> vds = new ArrayList<com.eastinno.otransos.container.annonation.Validator>();
     ...
     * @param vc
     */
    private void updateValidateObject(TargetObject targetObject, com.eastinno.otransos.container.annonation.Validator vc) {
     ...
     * 根据验证对象配置信息创建验证对象
     * 
     * @param vc 基于annotations的配置信息
     * @param target 属性所属的目标对象
     * @return 新创建或者已经存在的验证TargetObject
     */
    private TargetObject createValidateObject(com.eastinno.otransos.container.annonation.Validator vc, Object target) {
     ...
     * 
     * @param vc 基于annotations的验证对象配置信息
     * @param target 验证对象所属的目标对象
     * @return 具体的验证对象
     */
    private TargetObject addValidateObject(com.eastinno.otransos.container.annonation.Validator vc, Object target) {
