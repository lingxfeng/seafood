...
import com.eastinno.otransos.web.RequestProcessor;
import com.eastinno.otransos.web.WebConfig;
import com.eastinno.otransos.web.WebForm;
import com.eastinno.otransos.web.WebInvocationParam;
import com.eastinno.otransos.web.core.support.DefaultPageVender;
import com.eastinno.otransos.web.errorhandler.IErrorHandlerManager;
import com.eastinno.otransos.web.exception.DefaultExceptionHandle;
import com.eastinno.otransos.web.exception.FrameworkException;
import com.eastinno.otransos.web.exception.TokenVaildException;
import com.eastinno.otransos.web.exception.ValidateException;
import com.eastinno.otransos.web.interceptor.AfterInterceptor;
import com.eastinno.otransos.web.interceptor.BeforeInterceptor;
import com.eastinno.otransos.web.interceptor.ExceptionInterceptor;
import com.eastinno.otransos.web.interceptor.Interceptor;
 ...
 * </p>
 * <p>
 * Description:Disco框架的中心处理器,由ActionServlet调用,然后调用相应的Action并返回用户服务
 * </p>
 * <p>
...
public class DefaultRequestProcessor implements RequestProcessor {
    ...
    private DiscoFilter discoFilter; 
     ...
     * @param servlet
     */
    public DefaultRequestProcessor(DiscoFilter discoFilter, WebConfig webConfig) {
        this.discoFilter = discoFilter;
        this.webConfig = webConfig;
    }

    public void setFilter(DiscoFilter discoFilter) {
        this.discoFilter = discoFilter;
    }
    ...
    public void process(HttpServletRequest request, HttpServletResponse response) throws Throwable {
        try {
            ...
            if (vm != null)
                vm.cleanErrors();
            IPathMappingRuler mappingRuler = new PathMappingRulerImpl(request,this.webConfig.getModules());
            /**
             * 加入国际化环境
             */
            I18NResourceCache resource = new I18NResourceCache(FrameworkEngine.getResourcePath());
            ...
            doErrorHandler(e, ActionContext.getContext().getWebInvocationParam());
        }
    }

    @SuppressWarnings("unused")
    protected void process(HttpServletRequest request, HttpServletResponse response, ValidatorManager vm, IPathMappingRuler mappingRuler,
            WebForm webForm, int jumpTimes) throws FrameworkException, Throwable, Exception, ValidateException, IOException,
            ServletException {
        // 模块名称
        ...
        if (module != null) {
            ...
            // 创建WebForm
            WebForm form = webForm == null ? FrameworkEngine.creatWebForm(request, formName, module) : webForm;

            form.setUrl(ActionContext.getContext().getRequest().getHeader("REFERER"));
            // 取得命令，主要用于支持modern形式的url
            if (mappingRuler.getCommand() != null) {
                form.getTextElement().put("cmd", mappingRuler.getCommand());
            }
            // 装载页面参数
            ...
            if (action == null) {
                ...
                String lan = "core.web.did.not.find.the.class.with.the.template";
                throw new FrameworkException(I18n.getLocaleMessage(lan) + module.getAction() + " url:" + url);
            }
            if (form == null) {
            ...
            try {
                ...
                if (module.isValidate() && vm != null && vm.getErrors().hasError()) {
                    Page tempPage = page;
                ...
                // 执行验证处理
                form.getDiscoResult().putAll(form.getTextElement());// 把form中填写的数据存入结果集中
                page = findValidatePage(param);
                if (page == null)
                    throw ve;// 找不到验证错误页面，把异常交给异常拦截器统一处理
            }
            if (page != null) {
                Map result = form.getDiscoResult();
                ...
                if (page.getType().equals(Globals.PAGE_TEMPLATE_TYPE) || "string".equals(page.getType())) {
                    ...
                    processActionPage(request, response, vm, page, param, jumpTimes);
                    ...
                    if (gotoUrl.indexOf(':') < 0)
                        gotoUrl = request.getContextPath() + gotoUrl;
     ...
     * @throws ServletException
     */
    private void doVenderPage(Page page, WebForm form, HttpServletRequest request, HttpServletResponse response, WebInvocationParam param)
    ...
    /**
     * 处理action类型的Page对象 对于该类型对象，会记录已经跳转的次数，并在跳转次数达到框架设置的最大跳转次数后抛出FrameworkException。
     ...
     * @throws ServletException
     */
    private void processActionPage(HttpServletRequest request, HttpServletResponse response, ValidatorManager vm, Page page,
            WebInvocationParam param, int jumpTimes) throws FrameworkException, Throwable, Exception, ValidateException, IOException,
            ServletException {
        if (jumpTimes + 1 > this.webConfig.getMaxDirectJumpToActionTimes()) {
            throw new FrameworkException("has jump direct to another action more than " + this.webConfig.getMaxDirectJumpToActionTimes()
            ...
            PathMappingRulerImpl ruler = new PathMappingRulerImpl(request,this.webConfig.getModules());
            ruler.setUrlPattern(ActionContext.getContext().getWebInvocationParam().getUrlType());
            this.process(request, response, vm, ruler, param.getForm(), jumpTimes);
        }
    }
    ...
    protected Page findValidatePage(WebInvocationParam param) throws Exception {
        ...
        if (page == null) {
            String input = ActionContext.getContext().getRequest().getHeader("Referer");
    ...
    private void processAutoToken(WebInvocationParam param) throws Exception {
        ...
        if (m.getAutoToken()) {
            String tokenInRequest = (String) param.getForm().get(Globals.TOKEN_NAME);
     ...
     * 
     * @param error 程序执行过程中的异常
     * @param param 调用参数
     * @throws Exception 异常处理器出现异常，则表示异常将不由Disco处理
     */
    protected void doErrorHandler(Throwable error, WebInvocationParam param) throws Exception {
        ...
        while (it.hasNext()) {
            ...
            if (obj instanceof Map) {
                ...
                if (clz.isAssignableFrom(error.getClass())) {// 只有与指定exception的才可以处理
                    if (ei.get("class") != null) {
                        ...
                        if (gotoUrl.indexOf(':') < 0)
                            gotoUrl = ActionContext.getContext().getRequest().getContextPath() + gotoUrl;
            ...
            if (handlerClz != null && ExceptionInterceptor.class.isAssignableFrom(handlerClz) && handlerClz != DefaultExceptionHandle.class) {// 基于拦截器的异常处理方式
                ExceptionInterceptor ei = (ExceptionInterceptor) FrameworkEngine.getContainer().getBean(handlerClz);
                // 如果没有在容器中配置，则直接初始化一个
                if (ei == null)
                    ei = (ExceptionInterceptor) handlerClz.newInstance();
        ...
        ExceptionInterceptor defaultException = (ExceptionInterceptor) FrameworkEngine.getContainer().getBean(DefaultExceptionHandle.class);
        if (defaultException == null)
            defaultException = new DefaultExceptionHandle();
        defaultException.handle(error, param != null ? param.getAction() : null, null, new Object[] {param});
    }

    // 需要拦截非法输入 比如 ff' and 'fff 避免组合为SQL语句
    // 执行全局拦截操作,全局拦截必须是BeforeInterceptors
    // 还需要改动
    protected void invokeGlobalInterceptors(WebInvocationParam param) throws Throwable {
    ...
    protected void invokeActionBeforeInterceptors(Module module, WebInvocationParam param) throws Throwable {
        Iterator interceptors = module.getInterceptors().iterator();
        if (interceptors != null) {
            while (interceptors.hasNext()) {
                Interceptor interceptor = (Interceptor) interceptors.next();
                if (interceptor instanceof BeforeInterceptor)
                    ((BeforeInterceptor) interceptor).before(param.getAction(), null, new Object[] {param});
            }
        }
    }

    // 执行后拦截,后拦截类型为AfterInterceptors
    protected void invokeActionAfterInterceptors(Module module, Page page, WebInvocationParam param) throws Throwable {
        Iterator interceptors = module.getInterceptors().iterator();
        if (interceptors != null) {
            while (interceptors.hasNext()) {
                Interceptor interceptor = (Interceptor) interceptors.next();
                if (interceptor instanceof AfterInterceptor)
                    ((AfterInterceptor) interceptor).after(page, param.getAction(), null, new Object[] {param});
            }
        }
    }
    ...
    protected ServletContext getServletContext() {
        return (discoFilter.filterConfig.getServletContext());
    }
