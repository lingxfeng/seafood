...
/**
 * Disco内置的验证器 所有内置的验证器以内部类的方式提供，并在Disco启动的过程中自动加载。
 * 
 * @author lengyu
 */
public class Validators {
     ...
     * 在该验证器，参数expression值用来设定需要匹配的正则表达式，trim用来在匹配之前是否需要先进行去除空格的字符串处理操作，sensitive用来表示是否忽略大小写 使用方法
     * 
     * <pre>
    ...
    public class RegexpValidator extends AbstractValidator {
        ...
        public void validate(TargetObject obj, Object value, Errors errors) {
            ...
            boolean isCaseSensitive = obj.getValue("sensitive") != null ? true : false;
     ...
     * 该验证器可以设置以下的参数信息，min用来定义字符串的最小长度,min_msg定义当小于指定字符时的提示信息，max_msg用来定义当大于max字符时的提示信息
     * ，max用来定义字符串的最大长度，required表示必填，trim表示在验证前是否先去除左右空格，blank用来表示是否强制处理空格
     * 
     * <pre>
    ...
    public class StringValidator extends AbstractValidator {
        ...
        public void validate(TargetObject obj, Object value, Errors errors) {
            ...
            int length = v.length();
            Integer min = (Integer) new BeanWrapperImpl().convertIfNecessary(obj.getValue("min"), Integer.class);
            Integer max = (Integer) new BeanWrapperImpl().convertIfNecessary(obj.getValue("max"), Integer.class);
            if (min != null && length < min) {
                String minMsg = (String) obj.getValue("minMsg,min_msg");
                this.addError(obj, value, errors, minMsg != null ? minMsg : (String) obj.getValue("msg"));
            }
            if (max != null && length > max) {
                String maxMsg = (String) obj.getValue("maxMsg,max_msg");
                this.addError(obj, value, errors, maxMsg != null ? maxMsg : (String) obj.getValue("msg"));
            }
        }
    ...
    public class URLValidator extends AbstractValidator {
        ...
        public void validate(TargetObject obj, Object value, Errors errors) {
            ...
            if (!(value.getClass().equals(String.class)) || !TagUtil.verifyUrl((String) value)) {
                addError(obj, value, errors, (String) obj.getValue("msg"));
            }
        }
    }
    ...
    /**
     * Email格式字符串验证器，在系统中预定义的名称为email 用来要求目标值必须为正确的email格式字符。如ksmwly@gmail.com
     * 
     * @author lengyu
     */
    ...
    /**
     * 范围数据验证器，在系统中预定义的名称为range 该验证器用来要求目标值必须在特定范围之类，可以用于任何实现了Comparable接口的类，也包括Native类。
    ...
    public class RangeValidator extends RequiredValidator {
        ...
        public void validate(TargetObject obj, Object value, Errors errors) {
            ...
            if (obj.getValue("min") != null) {
                minComparatorValue = (Comparable) new BeanWrapperImpl().convertIfNecessary(obj.getValue("min"), value.getClass());
            }
            Comparable maxComparatorValue = null;
            if (obj.getValue("max") != null) {
                maxComparatorValue = (Comparable) new BeanWrapperImpl().convertIfNecessary(obj.getValue("max"), value.getClass());
            }
            // 检查数据是否小于指定的数据
            if ((minComparatorValue != null) && (v.compareTo(minComparatorValue) < 0)) {
                String minMsg = (String) obj.getValue("minMsg,min_msg");
                this.addError(obj, value, errors, minMsg != null ? minMsg : (String) obj.getValue("msg"));
            }
            // 指定数据是否大于指定的数据
            if ((maxComparatorValue != null) && (v.compareTo(maxComparatorValue) > 0)) {
                String maxMsg = (String) obj.getValue("maxMsg,max_msg");
                this.addError(obj, value, errors, maxMsg != null ? maxMsg : (String) obj.getValue("msg"));
            }
