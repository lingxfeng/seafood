 ...
 * </p>
 * cmd命令del(或Del)对应Action类的doDel方法
 * <p>
 ...
 * </p>
 * 1:35 2010-6-28 CHANGELOG 改变execute(WebForm form,Module module) 为 execute(WebForm f,Module m) ----DRY
 * 
 * @author lengyu
 * @version 1.0
 */
public abstract class AbstractCmdAction implements IWebAction {
    ...
    @InjectDisable
    private static final Logger logger = Logger.getLogger(AbstractCmdAction.class);
    @InjectDisable
    private static List<IActionCommandInterceptor> cmdInterceptors = new ArrayList<IActionCommandInterceptor>();
    ...
    protected Page forward(String command, boolean renew) {
        if (renew) {// 全新的跳转
            ...
            form.getTextElement().put("cmd", command);
            Module module = ActionContext.getContext().getWebInvocationParam().getModule();
            try {
    ...
    private String getUrl(String command) {
        String cmd = command/*
                             * , module = ActionContext.getContext() .getWebInvocationParam().getModule().getPath()
                             */;
     ...
     * @see com.eastinno.otransos.web.IWebAction#execute(com.eastinno.otransos.web.WebForm, com.eastinno.otransos.web.Module)
     */
    public Page execute(WebForm f, Module m) throws Exception {
        ...
        if (o != null) {
            if (o.getClass().isArray()) {
                String[] s = (String[]) o;
                StringBuffer sb = new StringBuffer(I18n.getLocaleMessage("core.web.execute")).append(this.getClass().toString()).append(
                        I18n.getLocaleMessage("core.web.found.a.number.of.commands"));
                for (int i = 0; i < s.length; i++) {
                    if (command == null && s[i] != null)
                        command = s[i];
                    sb.append(s[i]).append(",");
                }
                logger.info(sb.append(I18n.getLocaleMessage("core.web.from.here")).append(command)
                        .append(I18n.getLocaleMessage("core.web.value")));
            } else
                command = o.toString();
        }
        /*
         * command = CommUtil.null2String(f.get("cmd")); if ("".equals(command)) command =
         * CommUtil.null2String(f.get("cmd"));
         */
        try {
            ...
            if (mh != null) {
                if (cmdInterceptors.isEmpty())
                    cmdInterceptors = FrameworkEngine.getContainer().getBeans(IActionCommandInterceptor.class);
                // ActionCommand前拦截
                ...
                /*
                 * PermissionVerify pv=this.getClass().getAnnotation(PermissionVerify.class); RoleVerify rv =
                 * mh.getMethod().getAnnotation(RoleVerify.class); if((pv!=null
                 * &&!pv.disable())||rv!=null||FrameworkEngine.getWebConfig(). isPermissionVerify()){//执行权限查找
                 * if(permissionCheck==null){//首先从容器中尝试加载 permissionCheck=(IPermissionCheck)
                 * FrameworkEngine.getContainer ().getBean(IPermissionCheck.class);
                 * if(permissionCheck==null){//如果加载不到，则直接创建一个默认的权限检查器 permissionCheck=new DefaultPermissionCheck(); } }
                 * if(!permissionCheck .checkPermission(mh.getMethod())){//如果权限通不过 ActionContext.getContext
                 * ().getResponse().setHeader("Unauthorized", "true"); Page
                 * p=m.findPage("PERMISSION_PAGE");//允许自定义配置权限不足显示页面 if(p==null)p=newPage("PERMISSION_PAGE",
                 * "classpath:com/eastinno/otransos/web/exception/permissionError.html"); return p; } }
                 */
                ...
                if (objs.length > 0)
                    objs[0] = f;
                try {
                    WebCache cache = mh.getMethod().getAnnotation(WebCache.class);
                    if (cache != null) {
                        WebCacheManager manager = WebCacheManager.getInstance();
                        forward = manager.handleCache(ActionContext.getContext().getWebInvocationParam(), cache);
                    }
                    if (forward == null) {
                ...
                throw new Exception(I18n.getLocaleMessage("core.web.methods.name.is.not.correct.in") + this.getClass()
                        + I18n.getLocaleMessage("core.web.no.find") + command
                        + I18n.getLocaleMessage("core.web.method.Please.confirm.your.pages.cmd.or.cmd.in.the.parameter.values.are"));
    ...
    private MethodHolder findCmdMethod(Class actionClz, String command) throws NoSuchMethodException {
        String name = "do" + command.substring(0, 1).toUpperCase() + command.substring(1);
        MethodHolder ret = null;
        try {
            ret = findCmdMethod1(actionClz, name);// 查找doXxxx方法
        } catch (NoSuchMethodException e) {
            try {
                if (command.indexOf("do") == 0 && command.length() > 2 && Character.isUpperCase(command.charAt(2)))
                    throw e;
    ...
    private MethodHolder findCmdMethod1(Class actionClz, String name) throws NoSuchMethodException {
        Method method = null;
        Class[] paras = null;
    ...
    protected void addError(String name, String msg) {
        FrameworkEngine.findValidatorManager().addCustomError(name, msg, ValidateType.Action);
    }
    ...
    protected Map<String, Object> getErrors() {
        if (this.hasErrors())
            return new HashMap(FrameworkEngine.findValidatorManager().getErrors().getErrors());
        return new HashMap();
    }
     ...
     * 
     * @param logicName 模板逻辑名称
     * @param optionalPage 备选模板全路径
    ...
    /**
     * 保存一个token，用于防止重复提交 在需要防止重复提交的action中合适的地方添加代码saveToken() 在导向的页面的Form中添加$html.token来加入一个hidden域
    ...
    protected boolean isVaildateToken() {
        return token.isTokenValid((String) ActionContext.getContext().getWebInvocationParam().getForm().get(Globals.TOKEN_NAME));
    }
    ...
    protected boolean isVaildateToken(boolean reset) {
        return token.isTokenValid(reset, (String) ActionContext.getContext().getWebInvocationParam().getForm().get(Globals.TOKEN_NAME));
    }
    ...
    protected Page goBack() {
        ...
        String msg = CommUtil.null2String(form.getDiscoResult().get("msg"));
        if (!"".equals(msg)) {
            form.addResult("url", form.getUrl());
            return new Page("classpath:com/eastinno/otransos/web/tools/js.html");
        }
    ...
    public static void main(String[] args) {
        String s = "function(){var v0={};var v1={};v1['telphone']='手机号不能为空';v0['data']=null;v0['errors']=v1;v0['success']=false;return v0;}()";
    ...
    protected Page pageForExtForm(WebForm form) {
        ...
        if (this.hasErrors()) {
            r.setSuccess(false);
            Iterator it = this.getErrors().entrySet().iterator();
            while (it.hasNext()) {
                Entry en = (Entry) it.next();
                r.getErrors().put(en.getKey().toString(), en.getValue().toString());
            }
        }
        r.setData(form.getDiscoResult().get("javaObj"));
        form.jsonResult(r);
