...
import com.eastinno.otransos.web.FormProperty;
 ...
 * </p>
 * <p>
 * Description:读取XML格式的Disco框架配置文件信息；
 * </p>
 * <p>
...
public class XMLConfigFactory implements IConfigFactory {
    ...
    public XMLConfigFactory(InputStream in) {
        try {
            ...
            logger.warn(I18n.getLocaleMessage("core.web.unable.to.find.configuration.file") + Globals.CONFIG_FILE_FULL_PATH + ","
                    + I18n.getLocaleMessage("core.web.configuration.file.errors")
                    + I18n.getLocaleMessage("core.web.disco.will.automatically.use.the.default.path.processing.Action"));
        }
    }
    ...
    public void initForm(Map forms) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            // 设置错误提示类型，支持js及page两种方式，主要用于支持Disco代码自动生成工具
    ...
    public void initModule(Map modules) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            // 设置模板路径 如/hello.java 的路径为/hello
    ...
    public Map initOtherCfg() {
        ...
        if (root != null) {
            ...
            if (node != null)
                result.put(ConstantUtil.TEMPLATE_BASE_PATH, node.getText());
            // 处理框架初始化程序
        ...
        List importResources = new ArrayList();
        ...
        // 调用Disco的Bean理解处理器，将所有bean放置到 beanDefinitions中
