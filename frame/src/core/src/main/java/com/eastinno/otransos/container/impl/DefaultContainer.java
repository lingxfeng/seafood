...
public class DefaultContainer extends SingletonBeanContainer implements Container {
    ...
    public Object create(BeanDefinition beanDefinition) {
        ...
        if (bean != null) {
            ...
            for (int i = 0; i < beanDefinition.getPropertyValues().getPropertyValues().length; i++) {
                ...
                if (pv.getValue() instanceof BeanDefinition) {
                    ...
                    if (refBean == null)
                        throw new RuntimeException(I18n.getLocaleMessage("core.container.cant.load.bean.property.name.specified")
                                + beanName);
        ...
        // 在这里初始化容器信息
        if (bean != null && bean instanceof InnerContainer && !containers.containsKey(beanDefinition.getBeanName())) {
            logger.info(I18n.getLocaleMessage("core.container.loaded.a.container") + bean);
        ...
        if (Order.class.isAssignableFrom(type))
            Collections.sort(l, new Comparator() {
                public int compare(Object o1, Object o2) {
                    return ((Order) o1).getOrder().intValue() - ((Order) o2).getOrder().intValue();
                }
            });
        return l;
    }
