...
public abstract class AbstractWizardAction extends AbstractCmdAction {
    ...
    public Page execute(WebForm form, Module module) throws Exception {
        ...
        if (this.statusCheck && this.getCurrentStep() > 0) {
            ...
            if (!STATUS_OK.equals(status)) {
                ...
                form.getTextElement().put("cmd", this.commands[step]);
            }
        }
        ...
        if (forward == null && cmd != null && !"".equals(cmd)) {
            ...
            if (forward == null) {
                String templateUrl = module.getViews() + module.getPath() + "/" + cmd;
                String ext = Globals.DEFAULT_TEMPLATE_EXT;
                if (ext.charAt(0) == '.')
                    ext = ext.substring(1);
                forward = new Page(this.getClass() + cmd, templateUrl + "." + ext);
            }
        }
        return forward;
    }
    ...
    protected int getValidatePrevStep() {
        ...
        if (this.statusCheck) {
            for (int j = 0; j > i; j++) {
                if (!STATUS_OK.equals(this.getStatus(j)))
                    return j;
            }
        }
        return i - 1;
    ...
    protected String getPreviousCommand() {
        return getCurrentStep() <= 0 ? null : this.commands[this.getCurrentStep() - 1];
    }
    ...
    protected String getNextCommand() {
        return !isFinished() ? this.commands[this.getCurrentStep() + 1] : null;
    }
