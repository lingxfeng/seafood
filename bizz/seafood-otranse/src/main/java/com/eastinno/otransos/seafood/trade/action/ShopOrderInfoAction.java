...
import com.eastinno.otransos.seafood.util.LogisticsVo;
...
public class ShopOrderInfoAction extends WxShopBaseAction {
	...
	public Page doViewList(WebForm form) {
		...
		String code = CommUtil.null2String(form.get("orderCode"));
	 ...
	 * @return
	 */
	public Page doSendCustumOrder(WebForm form) {
		String orderId = CommUtil.null2String(form.get("orderId"));
		ShopOrderInfo order = this.service.getShopOrderInfo(Long.parseLong(orderId));
		if (order.getShopSinceSome() != null && order.getStatus() == 1) {
			order.setStatus(2);
			order.setSendDate(new Date());
			this.service.updateShopOrderInfo(order.getId(), order);
		}
		return go("receiveList");
	}
	...
	public Page doOrderDetail2(WebForm form) {
		...
		form.addResult("order", order);
		/*
		 * if(order.getStatus()==0){ QueryObject qo = new QueryObject();
		 * qo.addQuery("obj.type",PayTypeE.WEIXINMPAPI,"="); qo.setPageSize(1);
		 * List<PaymentConfig> configs =
		 * this.paymentConfigService.getPaymentConfigBy(qo).getResult();
		 * if(configs!=null && configs.size()>0){ PaymentConfig config =
		 * configs.get(0); String reMsg =
		 * this.shopPayMentService.paySubmit(order, config, "");
	...
	public Page doCancel(WebForm form) {
		...
		for (ShopOrderdetail orderdetail : list2) {
			System.out.println("=======shopOrderinfoAction  docancel执行了=====");
	...
	public Page doSendPro(WebForm form) {
		...
		String expressNote = CommUtil.null2String(form.get("expressNote"));
		String expressid = CommUtil.null2String(form.get("expressid"));
		if (expressCode != null && !expressCode.equals("") && expressid != null && !expressid.equals("")) {
			...
			order.setStatus(2);
			order.setSendDate(new Date());
			order.setExpressNote(expressNote);
			...
			if (f != null) {
				...
				WeixinBaseUtils.sendMsgToFollower(a, f,
						"【舟山海鲜之家控股有限公司】尊敬的客户" + member.getNickname() + "您好！您的订单:" + order.getCode() + "已经发出，"
								+ order.getLogisticsCompany().getName() + "快递单号：" + order.getExpressCode() + "(备注："
								+ expressNote + ")，请保持电话通畅，您可在商城的订单内查询实时物流信息，收货时请当面验货。也可随时致电400-9261088查询物流信息。");
			}
		}
		return go("sendList");
	}
	 ...
	 * @return
	 */
	public Page doChangeLogis(WebForm form) {
		String orderId = CommUtil.null2String(form.get("orderId"));
		String idStr = CommUtil.null2String(form.get("id"));
		String code = CommUtil.null2String(form.get("codestr"));
		if (StringUtils.hasText(idStr)) {
			ShopOrderInfo order = this.service.getShopOrderInfo(Long.parseLong(orderId));
			if (StringUtils.hasText(idStr)) {
				LogisticsCompany company = this.logisticsCompanyService.getLogisticsCompany(Long.parseLong(idStr));
	 ...
	 * @return
	 */
	public Page doSearchKuaidi(WebForm form) {
		String orderId = CommUtil.null2String(form.get("orderId"));

		if (StringUtils.hasText(orderId)) {
			ShopOrderInfo order = this.service.getShopOrderInfo(Long.parseLong(orderId));
			form.addResult("name", order.getLogisticsCompany().getName());
			form.addResult("num", order.getExpressCode());
			HashMap param1 = new HashMap();
			param1.put("com", order.getLogisticsCompany().getCode());
			param1.put("num", order.getExpressCode());
			param1.put("phone", order.getAddr().getTelephone());
			// String param =
			// "{\"com\":\"shunfeng\",\"num\":\"764767994970\",\"phone\":\"13819789308\",\"from\":\"\",\"to\":\"\","resultv2":0}";
			String param = JSON.toJSONString(param1);
			String customer = "23BFB7314E72811518BA2E58DC3419F1";
			String key = "MzgrmEHU5512";
			...
			try {
				...
				post = new HttpPost("http://poll.kuaidi100.com/poll/query.do");
				List<BasicNameValuePair> nvps = new ArrayList<BasicNameValuePair>();
				for (String name : params.keySet()) {
					BasicNameValuePair newp = new BasicNameValuePair(name, params.get(name));
					nvps.add(newp);
				}
				...
				if (statusCode != HttpStatus.SC_OK) {
					form.addResult("msg", "fail");
				} else {
					String result = convertStreamToString(response.getEntity().getContent());
					System.out.println(result);
					JSONObject obj = JSON.parseObject(result);
					if (obj != null && obj.get("status").toString().equals("200")) {
						List<LogisticsVo> list = JSON.parseArray(obj.getString("data"), LogisticsVo.class);

						form.addResult("list", list);
						form.addResult("msg", "success");
					} else {
						form.addResult("msg", "fail");
					}
				}
		...
		return new Page("/bcd/wxshop/member/logic.html");
