...
public class ShoppingCartAction extends WxShopBaseAction {
	...
	public Page doToShoppingCart(WebForm form) {
		...
		if (member != null) {
			...
			qo.addQuery("obj.shopProduct.status", Short.parseShort("1"), "=");
			qo.setLimit(-1);
	...
	public Page doAddToCart(WebForm form) {
		...
		// ProductType
		// pType=this.productTypeService.getProductTypeByCode(ptId[1]);
		...
		// /ShopDistributor
		// distributor=(ShopDistributor)ActionContext.getContext().getSession().getAttribute("DISTRIBUTOR");
