...
public class ShopPayMentServiceImpl implements IShopPayMentService, IPayCallOrderService {
	...
	@Override
	public String paySubmit(ShopOrderInfo orderInfo, PaymentConfig payconfig, String defaultbank) {
	 ...
	 */
	@Override
	public String paySubmit2(ShopOrderInfo orderInfo, PaymentConfig payconfig, String defaultbank,
	...
	@Override
	public String paySubmitByAmt(ShopOrderInfo orderInfo, PaymentConfig payconfig, String defaultbank,
			CustomCoupon customCoupon, ShopMember member) {
		orderInfo.setUuid(new Date().getTime() + "");
		orderInfo.setPayConfig(payconfig);
		if (customCoupon != null) {
			Double cValue = customCoupon.getCoupon().getValue();
		...
		if (rAmt >= gPri) {
			...
			/*
			 * BigDecimal amt = new
			 * BigDecimal(Double.toString(member.getRemainderAmt())); BigDecimal
			 * price = new
			 * BigDecimal(Double.toString(orderInfo.getGross_price()));
			 * member.setRemainderAmt(amt.subtract(price).doubleValue());
			 * this.shopMemberService.updateShopMember(member.getId(), member);
			 */
			// this.remainderAmtHistoryService.addRemainderAmtHistory(member, 1,
			// orderInfo.getGross_price(), member.getNickname()+"购物消费余额");
			return "0";
		} else {
			...
			// this.remainderAmtHistoryService.addRemainderAmtHistory(member, 1,
			// member.getRemainderAmt(), member.getNickname()+"购物消费余额");
	 ...
	 */
	@Override
	public String paySubmitByAmt2(ShopOrderInfo orderInfo, PaymentConfig payconfig, String defaultbank,
			ShopMember member) {
		orderInfo.setUuid(new Date().getTime() + "");
		orderInfo.setPayConfig(payconfig);
		String total_fee = "";
	 ...
	 * @return
	 */
	public String rechargePaySubmit(IntegralRechargeRecord rechargeRecord, PaymentConfig payconfig,
			String defaultbank) {
	...
	public void updateRecord(PayReturnObj payreturn) {
		...
		qo.addQuery("obj.uuid", uuid, "=");
		qo.setPageSize(1);
		List<IntegralRechargeRecord> list = this.integralRechargeRecordService.getIntegralRechargeRecordBy(qo)
				.getResult();
		if (list != null && list.size() > 0) {
			...
			if (irr != null && irr.getStatus() != 1) {
				...
				this.integralHistoryService.saveIntegralHistory(irr.getIntegral(), member,
						member.getNickname() + "充值获取积分", 2);// 积分历史记录
			}
		}
	}

	@Override
	public String paySubmit3(IntegralRechargeRecord integralRechargeRecord, PaymentConfig payconfig,
			String defaultbank) {
		PayParamsObj payParams = new PayParamsObj();
