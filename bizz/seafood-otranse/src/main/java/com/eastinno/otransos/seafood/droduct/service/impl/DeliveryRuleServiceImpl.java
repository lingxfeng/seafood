...
public class DeliveryRuleServiceImpl implements IDeliveryRuleService {
	...
	public Double getDeliveryCost(ShopOrderInfo orderInfo) {
		 ...
		 * if(orderInfo.getGross_price().compareTo(Double.parseDouble(tempRule.
		 * getRuleValue())) > 0){ return result; }
		 */
	...
	private Double getNearDeliveryCostByOrderDetails(List<ShopOrderdetail> details) {
		...
		Integer extFeight = 0;// 定制运费的结果
		// 依据detail，获取规则数组
		int orderNum = details.size();
		if (details != null) {
			for (int i = 0; i < orderNum; ++i) {
				// 如果发现定制邮费的商品，都按照定制邮费的商品进行计算邮费
				try {
					ShopProduct product = this.shopProductService.getShopProduct(details.get(i).getPro().getId());
					DeliveryRuleExt ext = product.getDeliveryRuleExt();
					if (ext != null) {
						Integer productNum = details.get(i).getNum();
						Integer temExtFeight = 0;
						if (ext.getLevel4Num() != 0 && productNum >= ext.getLevel4Num()) {
							temExtFeight = ext.getLevel4NearValue();
						} else if (ext.getLevel3Num() != 0 && ext.getLevel4Num() != 0
								&& productNum >= ext.getLevel3Num() && productNum < ext.getLevel4Num()) {
							temExtFeight = ext.getLevel3NearValue();
						} else if (ext.getLevel2Num() != 0 && ext.getLevel3Num() != 0
								&& productNum >= ext.getLevel2Num() && productNum < ext.getLevel3Num()) {
							temExtFeight = ext.getLevel2NearValue();
						} else if (ext.getLevel1Num() != 0 && ext.getLevel2Num() != 0
								&& productNum >= ext.getLevel1Num() && productNum < ext.getLevel2Num()) {
							temExtFeight = ext.getLevel1NearValue();
						}

						if (temExtFeight.compareTo(extFeight) > 0) {
							extFeight = temExtFeight;
						}
					}
				} catch (Exception e) {
					e.printStackTrace();
		...
		Double dExt = Double.parseDouble(extFeight + "");
		if (dExt.compareTo(cost) > 0) {
			cost = dExt;
		}
		return cost;
	}
			...
			for (int i = 0; i < orderNum; ++i) {
				...
				try {
					...
					if (ext != null) {
						...
						Integer temExtFeight = 0;
						if (ext.getLevel4Num() != 0 && productNum >= ext.getLevel4Num()) {
							temExtFeight = ext.getLevel4FarValue();
						} else if (ext.getLevel3Num() != 0 && ext.getLevel4Num() != 0
								&& productNum >= ext.getLevel3Num() && productNum < ext.getLevel4Num()) {
							temExtFeight = ext.getLevel3FarValue();
						} else if (ext.getLevel2Num() != 0 && ext.getLevel3Num() != 0
								&& productNum >= ext.getLevel2Num() && productNum < ext.getLevel3Num()) {
							temExtFeight = ext.getLevel2FarValue();
						} else if (ext.getLevel1Num() != 0 && ext.getLevel2Num() != 0
								&& productNum >= ext.getLevel1Num() && productNum < ext.getLevel2Num()) {
							temExtFeight = ext.getLevel1FarValue();
						}
	...
	private Double getNoneRemoteByOrderDetails(List<ShopOrderdetail> details) {
		...
		if (details != null) {
			...
			for (int i = 0; i < detailNum; ++i) {
				...
				try {
					...
					if (ext != null) {
						...
						Integer temExtFeight = 0;
						if (ext.getLevel4Num() != 0 && productNum >= ext.getLevel4Num()) {
							temExtFeight = ext.getLevel4CommonValue();
						} else if (ext.getLevel3Num() != 0 && ext.getLevel4Num() != 0
								&& productNum >= ext.getLevel3Num() && productNum < ext.getLevel4Num()) {
							temExtFeight = ext.getLevel3CommonValue();
						} else if (ext.getLevel2Num() != 0 && ext.getLevel3Num() != 0
								&& productNum >= ext.getLevel2Num() && productNum < ext.getLevel3Num()) {
							temExtFeight = ext.getLevel2CommonValue();
						} else if (ext.getLevel1Num() != 0 && ext.getLevel2Num() != 0
								&& productNum >= ext.getLevel1Num() && productNum < ext.getLevel2Num()) {
							temExtFeight = ext.getLevel1CommonValue();
						}
			...
			if (extFeight > 0) {
				System.out.println("====发现当前商品有定制运费：" + extFeight);
				return Double.parseDouble(extFeight + "");
			}
			Double countAmt = 0D;
