...
public class ShopManageAction extends ShopBaseAction{
	...
	public Page doToMap(WebForm form){
		String flag = CommUtil.null2String(form.get("flag"));
		form.addResult("level1map",this.shopDistributorService.getMapLevel1Date());
		form.addResult("level2map",this.shopDistributorService.getMapLevel2Date());
		if(flag.equals("1")){
			return new Page("/shopmanage/entityShopMap.html");
		}else if(flag.equals("2")){
			return new Page("/shopmanage/virtualShopMap.html");
		}else{
			return new Page("/shopmanage/ShopMap.html");
		}
