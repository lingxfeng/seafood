var pagestyle=function(){    
   ...
   try{ 
		...
		var h = jQuery(window).height() - iframe.offset().top-35;
		var w = jQuery(window).width() - iframe.offset().left-15;
...
function openURL(args){
 ...
 if(type=="show"){
   ...
   jQuery("#top_nav_info").html(first_a.text());
 ...
 if(type=="show_complex"){
   ...
   jQuery(".nav a").removeClass("home");
   jQuery("#"+item_id+"_menu").addClass("this"); 
   var first_li=jQuery("#"+item_id+" ul li").first();
   var first_a=jQuery("#"+item_id+" ul li a").first();
   jQuery(".ulleft a").removeClass("this");
   first_a.addClass("this");
   jQuery("#top_nav_info").html(first_a.text());
   var html=first_li.html().match(/openURL\('(.*)'\)/ig);
 ...
 if(type=="url"){
  if(parent_id!=undefined){
   ...
   jQuery("#top_nav_info").html(jQuery("#"+url_id).text());
 }
 //
 pagestyle();
}
